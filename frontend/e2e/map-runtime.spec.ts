...
// ページはピンチで拡大させない（touch-action）が、地図の上で始めたピンチは地図の拡大として
// 届き続けなければならない（パターン4 観点2）。地図のcanvas以外の部品のピンチは
// all-states.spec.ts が全状態で見る。touch-actionはタッチの始点の要素と祖先で決まり、計算後の
// CSSの値を読むだけでは効いているかが分からないので、実際に2本指のタッチを送って見る。
test.describe("タッチ端末", () => {
