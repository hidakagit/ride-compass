...
describe("useLocation", () => {
  ...
  it("空欄のままhandleManualSubmitすると(0,0)へ切り替わらずエラーになる", () => {
    // Number("")は0を返しNaNにならないため、以前は空欄のまま送信すると気づかず
    // (0,0)（ギニア湾沖）へ切り替わっていた。
    const { result } = renderHook(() => useLocation());

    act(() => {
      result.current.setManualLat("");
      result.current.setManualLng("");
    });
    act(() => {
      result.current.handleManualSubmit({ preventDefault: () => {} } as React.FormEvent);
    });

    expect(result.current.location).not.toEqual({ latitude: 0, longitude: 0 });
    expect(result.current.manualLocationError).toBe("緯度・経度を入力してください。");
  });

  it("範囲外の緯度経度をhandleManualSubmitするとエラーになり位置は更新されない", () => {
    const { result } = renderHook(() => useLocation());
    const before = result.current.location;

    act(() => {
      result.current.setManualLat("999");
      result.current.setManualLng("135.0");
    });
    act(() => {
      result.current.handleManualSubmit({ preventDefault: () => {} } as React.FormEvent);
    ...
    expect(result.current.location).toEqual(before);
    expect(result.current.manualLocationError).toMatch(/-90〜90/);
  });
});
