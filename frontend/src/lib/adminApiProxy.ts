// 軸CRUD管理API（backend/app/api/routers/axis_admin.py）のサーバー側プロキシ（改善計画T305）。
// `frontend/src/app/admin/api/axis-definitions/`配下の各route handlerからのみ呼ぶこと
// （route handlerは常にサーバー側実行のため、"use client"コンポーネントから直接importしない
// 限りADMIN_BASIC_AUTH_PASSWORD等がブラウザバンドルへ漏れる心配はない）。
//
...
//
// 代わりに、この一群のroute handler（`/admin/api/axis-definitions/...`、proxy.tsのmatcher
// `/admin/:path*`に含まれるパス）を経由する。ブラウザは/admin読込時に一度Basic認証すれば、
// 同一オリジン・同一realmへの後続リクエストの認証情報を自身の認証キャッシュから自動付与する
// （fetchの既定のcredentialsモード"same-origin"がこれを含む、ブラウザ標準の挙動）ため、
// クライアント側のJSは何もしなくてよい。このNext.jsサーバー（route handler）が、
// サーバー環境変数ADMIN_BASIC_AUTH_USERNAME/PASSWORD（proxy.tsが既に使っている値と同じ、
 ...
 * `request.method`をそのまま使う）。 */
export async function proxyToAxisAdmin(request: Request, backendPath: string): Promise<Response> {
  ...
  try {
    backendResponse = await fetch(`${BACKEND_INTERNAL_URL}${backendPath}`, {
