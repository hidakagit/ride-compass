...
// レイヤーを追加するときは、源泉（backendの`domain/map_display.py`）へ種別・情報源・性質・
// 既定表示を宣言し、ここへ見せ方（名前・アイコン・説明・表示専用の凡例`readOnlyLegend`）の
// MapLayerDescriptor を1つ足す。画面側（features/map/view・MapOverlayControls）は変更しない。
//
// 地図に描く宣言（features/map/scene/groups/ の家族。絞り込める凡例は scene/legends.ts）は
// 別途必要で、**ここが1箇所では済まない**。書き忘れるとチップはONになり凡例も出るのに
// 地図には何も出ない。
//
// categoryは▶パネル（MapOverlayControls）のグループ見出しに使う中分類。flatな一覧のまま
// 並べると、増えるほど見つけにくくなるため分ける。区分と各レイヤーの所属は源泉が持ち、
// 並び順は`MAP_LAYER_CATEGORY_ORDER`が決める。例: 道路状態（道路の種類・路面の種類）、
...
export interface MapLayerDescriptor {
  ...
  /** 絵の出所（`MapLayerDataSource`）。源泉の宣言から入る（`declaredLayer`）。
   *
   ...
   * （UI語彙のカタログ集約）。 */
  panelHint?: string;
  /** 利用者の操作を待たずに既定で表示するか。源泉の宣言から入る。
   * `buildDefaultLayerVisibility`がここから初期値を導く。 */
  defaultOn?: boolean;
  /** 配信元のタイルがこのズーム未満では要求されない（ONにしても地図には何も出ない）。
   * 情報源の宣言から入り、ズーム不足の間チップに案内を出し凡例を空にする扱いが付く
   * （`tileZoomTooWideLayerIds`）。下限を持たない情報源のレイヤーは持たない。 */
  tileMinZoom?: number;
}
...
export function buildMapLayers(
  ...
  return [
    ...
    // 凡例（段階・色・絞り込み）は`scene/legends.ts`と`view/lens.ts: buildAxisRampLegend`が
    // 他の静的レイヤーと同じ仕組みで提供する。
    ...
    })),
    {
      ...
      // ラスタタイルで重ね描きする。その先は源泉が宣言する時刻の段（降水短時間予報、
      // 風と共有の格子点マップ由来の降水量を格子ごとに塗るgridFill表現）へ内部で
    ...
    })),
    {
      ...
      // キキクルは「現在の危険度」単一値のみの配信のため連動しない（源泉の要素ごとの規則）。
 ...
 *
 * **世代を配るタイルの系統がそのまま情報源の名前**（源泉の`map_display.py`がタイルの系統を
 * 情報源の名前に使う）なので、系統の一覧から導く。国土地理院のラスタ・土地被覆ラスタは
 * 世代を持たない別系統のため入らない。 */
