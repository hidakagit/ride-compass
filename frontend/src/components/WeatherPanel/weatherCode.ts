...
// 改善計画T385: Open-MeteoのWMO天気コード（weather_code）+ is_dayから、天候ヘッダーの
// 天気アイコン1個を決める。以前はUV指数を専用チップ（アイコン+数値）で常時表示していたが、
// UV指数は夜間常に0.0になり情報価値が無いままヘッダーを圧迫していた（ユーザー指摘
// 「収まり方が不自然」）。weather_codeなら昼夜を問わず常に意味のある値（快晴/くもり/雨等）
// が取れるため、UV指数の置き換えとして採用する。UV指数自体は数値としての価値が残るため
