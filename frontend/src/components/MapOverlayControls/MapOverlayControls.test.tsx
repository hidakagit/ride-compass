...
describe("MapOverlayControls", () => {
  ...
  describe("次数束ね（改善計画T166）", () => {
    ...
    // 改善計画T334（実機フィードバック「▶内に移動した説明文は消して」の直後に依頼されていた
    // 「表示する項目を選ぶ」ウィンドウ内、各項目への情報アイコン追加が漏れていた不具合への
    // 対応）。観測・動的の両グループで同じ`renderVisibilitySettings`を通るため、同じ検証を
    // 両方に対して行い「作りが同じ」であることを裏付ける。
    it.each([
      { groupName: "観測", settingsLabel: "観測データの表示項目を設定", memberId: "designation", memberLabel: "指定路線" },
      { groupName: "動的", settingsLabel: "動的データの表示項目を設定", memberId: "windVector", memberLabel: "風" },
    ])(
      "$groupNameグループの表示項目設定で、説明文(panelHint)を持つ項目には情報アイコンが出て、押すと説明文が開閉する",
      async ({ settingsLabel, memberId, memberLabel }) => {
        const user = userEvent.setup();
        const layers: OverlayLayerChip[] = [
          {
            id: memberId as OverlayLayerChip["id"],
            label: memberLabel,
            on: false,
            category: "trafficSafety",
            dataNature: memberId === "windVector" ? "dynamic" : "raw",
        ...
        // panelHintを持つ項目には情報アイコンが出る
        const infoButton = screen.getByRole("button", { name: `${memberLabel}の説明を表示` });
        expect(infoButton).toBeInTheDocument();
        expect(screen.queryByText("これはテスト用の説明文です。")).not.toBeInTheDocument();

        // panelHintを持たない項目には情報アイコンが出ない
        expect(screen.queryByRole("button", { name: "事故地点の説明を表示" })).not.toBeInTheDocument();

        await user.click(infoButton);
        expect(screen.getByText("これはテスト用の説明文です。")).toBeInTheDocument();
        expect(screen.getByRole("button", { name: `${memberLabel}の説明を隠す` })).toBeInTheDocument();

        await user.click(screen.getByRole("button", { name: `${memberLabel}の説明を隠す` }));
        expect(screen.queryByText("これはテスト用の説明文です。")).not.toBeInTheDocument();
      }
    );

    // 推定グループはsecondaryAxes props（axis.panelHint）から情報アイコン用の説明文を
    // 引く点だけが観測/動的と異なる（データの取得元、renderVisibilitySettings自体は共通）。
    // SECONDARY_AXES（静的フォールバック）のstop_density軸が実際にpanel_hintを持つため、
    // それをそのまま使う（実データでの回帰確認、カスタムフィクスチャ不要）。
    it("推定グループの表示項目設定でも、panelHintを持つ軸には情報アイコンが出て説明文が開閉する", async () => {
      ...
      expect(infoButton).toBeInTheDocument();
      // 勾配軸はpanel_hintがnullのため情報アイコンが出ない
    ...
    it("推定グループの各軸タイルの▼を開くと材料一覧が出て、表示レイヤーの有無で分けて表示される", async () => {
      ...
      // 車の圧迫感: 道路種別・指定路線はレイヤーあり、制限速度・車線数・車両可否は無し
      // 改善計画T320: 一次属性の解決順がAXIS_DEFINITIONSの内部軸材料の走査順（highway→
      // maxspeed→lanes→designation→motor_vehicle_access）に一本化されたため、
      // 未表示材料の並びも制限速度→車線数の順になった（以前はregistry_defaults.pyの
      // 手書きinputs順が偶然lanes→maxspeedだった）。
      // 改善計画T353: car_stress_bicycle_infra_adjustmentを廃止（1材料1軸原則）した
      // ため、cyclewayはcar_stressの材料一覧そのものから外れた（T347時点は「未表示」側に
      // 残っていたが、T353で軸の材料一覧から完全に削除された）。
