...
describe("MapOverlayControls", () => {
  ...
  describe("動的グループ（改善計画T171）", () => {
    ...
    // ユーザー判断（2026-08-25）: 動的グループは絞り込み機能を持たないため「地図の見え方」
    // パネルの行自体を撤去した（MapLayersPanel.test.tsx参照）。そこで見せていた説明文
    // （panelHint、雷ナウキャストの安全上の注意を含む）の行き場をここへ移した
    // （page.tsx: overlayLayersのhint、MapOverlayControls.tsx: renderRawMemberTile参照）。
    it("hintがあれば凡例（legendDetails）と併せて▶パネルに常に表示される", async () => {
      const user = userEvent.setup();
      const layers: OverlayLayerChip[] = [
        {
          id: "thunderNowcast",
          ...
          category: "weather",
          dataNature: "dynamic",
          hint: "活動度2以上が表示されている領域では、直ちに建物の中など安全な場所への避難が必要です。",
          legendDetails: [
            { label: "", legend: [{ key: "1", label: "活動度1", color: "#fde68a", filter: ["literal", true] }], hiddenKeys: [] },
          ],
        },
      ];
      render(<MapOverlayControls {...baseProps()} layers={layers} />);
      await user.click(screen.getByRole("button", { name: "動的" }));
      await user.click(screen.getByRole("button", { name: "雷ナウキャストの凡例を表示" }));

      // 凡例（帯一覧）と説明文の両方が同時に読める（どちらか一方に抑制されない）。
      expect(screen.getByText("活動度1")).toBeInTheDocument();
      expect(screen.getByText(/直ちに建物の中など安全な場所への避難が必要です/)).toBeInTheDocument();
    });

    // 実機フィードバック（2026-08-25）「動的アイコンの（！）を押すと開くポップアップの
    // 中で、さらに（！）を出して」への対応。折りたたみ中は個々のメンバータイル（▶）に
    // 辿り着けないため、「表示する項目を選ぶ」設定パネル自体からも各メンバーの説明文を
    // 確認できるようにした。
    it("折りたたみ中の「表示する項目を選ぶ」設定パネルでも、各項目の（！）から説明文を確認できる", async () => {
      const user = userEvent.setup();
      const layers: OverlayLayerChip[] = [
        {
          id: "thunderNowcast",
      ...
      render(<MapOverlayControls {...baseProps()} layers={layers} />);
      // グループ本体は開かず、折りたたみ時だけ出る設定パネルの入口を押す。
      await user.click(screen.getByRole("button", { name: "動的データの表示項目を設定" }));
      expect(screen.queryByText(/直ちに建物の中など安全な場所への避難が必要です/)).not.toBeInTheDocument();

      // 設定パネル内の項目ラベルはchipLabel（略名、ここでは「雷」）を使う。
      await user.click(screen.getByRole("button", { name: "雷の説明を表示" }));
      expect(screen.getByText(/直ちに建物の中など安全な場所への避難が必要です/)).toBeInTheDocument();
    });
  });
});
