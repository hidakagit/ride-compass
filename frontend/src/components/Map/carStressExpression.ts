// 車ストレス（1-5）をタイルの材料タグ（backend/app/infrastructure/road_graph_repository.py:
...
//
// 改善計画（車ストレスレシピ外出し基盤）: 以前はSQL側で最終値を計算しタイル（全ユーザー
...
// JSONファイル名（traffic-stress-recipe.json）は改善計画T150の「交通ストレス→車ストレス」
// 改称に未追従のまま据え置いている（backend/scripts/export_openapi.py:
// TRAFFIC_STRESS_RECIPE_PATHが書き出すファイル名そのもの）。フロント側だけ先行してimportパスを
...
import defaultRecipeJson from "@/types/generated/traffic-stress-recipe.json"; // T150未追従の意図的な旧ファイル名（コメント参照）
...
export type CarStressRecipe = CarStressRecipeOverride;
...
export const DEFAULT_CAR_STRESS_RECIPE: CarStressRecipe = defaultRecipeJson;
...
export function buildCarStressExpression(
  ...
  // 該当しない（domain/traffic.py: car_stress_breakdownと1:1対応）。lanes_high
...
export function evaluateCarStressLevel(
  properties: Record<string, unknown>,
  recipe: CarStressRecipe = DEFAULT_CAR_STRESS_RECIPE,
  roadSuitabilityRecipe: RoadSuitabilityRecipe = DEFAULT_ROAD_SUITABILITY_RECIPE,
  motorVehicleDensityRecipe: MotorVehicleDensityRecipe = DEFAULT_MOTOR_VEHICLE_DENSITY_RECIPE,
  carCloseness: CarClosenessExpr = carClosenessExpr(roadSuitabilityRecipe, motorVehicleDensityRecipe),
): number | null {
  return evaluateRecipeLevel(
    buildCarStressExpression(recipe, roadSuitabilityRecipe, motorVehicleDensityRecipe, carCloseness),
    properties,
    "車の圧迫感",
  );
}
