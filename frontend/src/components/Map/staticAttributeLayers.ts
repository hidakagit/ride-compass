...
// 生データ加工として持たせない設計原則のため、既に評価軸の材料として使われている
// 正規化フラグ4種（highway_is_cycleway等）だけを正準とする。
//
...
function buildCategoricalLayerDefs(
  ...
  // （roadFilterAxes.tsのFALLBACK_LINE_OPACITY/KNOWN_LINE_OPACITYと共有し、地図全体で
  // 読み方を統一する）。
...
//
// 当事者（自転車関連/その他）は「事実の種類の区別であり重大度ではない」ように見えるが、
...
// 事故の「重大度」絞り込み軸。当事者（自転車関連/その他、ACCIDENT_LEGEND）とは
...
// 他のカテゴリ色（当事者・停止要因種別等）は「事実の種類」を区別するラベルにすぎないが、
// 死亡事故か否かはそれ自体が重大な事実であり、赤＝危険という慣習的な読みが安全確認という
// 目的に直接寄与する（円の拡大ACCIDENT_RADIUS_EXPRESSIONと合わせて二重に強調する設計）。
...
export interface StaticFilterAxis {
  ...
  /** 凡例の非表示操作の有無に関わらず常にANDする恒常的な絞り込み
