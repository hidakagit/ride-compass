...
import { installImageDataPolyfill } from "@/testing/imageDataPolyfill";

// 描画内容ではなく、関数が例外なく呼び出せることを確認するスモークテスト。
...
describe("createWindArrowIcon（描画コードパスの実行検証）", () => {
  function stubCanvasContext() {
    ...
    const ctx = {
      ...
      moveTo: vi.fn<(x: number, y: number) => number>(() => calls.push("moveTo")),
      lineTo: vi.fn<(x: number, y: number) => number>(() => calls.push("lineTo")),
