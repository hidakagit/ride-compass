...
export const MAP_LAYERS: readonly MapLayerDescriptor[] = [
  ...
  {
    // 気象庁 降水ナウキャスト（改善計画T170/T171、T183で延長予報を追加）。実況（過去〜現在、
    // 5分毎）と60分先までの短時間予測をラスタタイルで重ね描きする。60分より先（ユーザー要望
    // 「1時間より先も、短時間雨予報を出してほしい」）は、風と共有の格子点マップ由来の
    // 自前実装アイコン（precipitationNowcast.ts/MapView.tsx: PRECIPITATION_EXTENDED_*）へ
    // 内部で切り替わる。表示・トグルは1つのまま（ユーザー要望「アイコンは1つ。ただし内部は
    // 時間によって使い分けて」）。他の静的レイヤーと異なり、表示中の時刻を地図上の時刻
    // スライダー（layerVisibility.precipitationNowcastがONの間だけ表示、page.tsx参照）で
    // 切り替えられる。
    ...
    panelHint:
      ...
      "風と同じ仕組み（Open-Meteo経由の格子点予報）による延長予報アイコンへ自動的に切り替わり、" +
      "約48時間先まで確認できます（直近ほどの精度ではない粗いモデル予報です）。非公式の内部APIを" +
