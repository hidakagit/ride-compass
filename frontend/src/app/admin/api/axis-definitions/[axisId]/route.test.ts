...
// paramsからaxisIdを取り出し、encodeURIComponentしたうえでproxyToAxisAdminへ委譲することを
// 確認する（proxyToAxisAdmin自体の挙動はlib/adminApiProxy.test.tsで検証済み）。
...
describe("PUT/DELETE /admin/api/axis-definitions/[axisId]", () => {
  it("PUTはaxisIdをpathへ埋め込みproxyToAxisAdminへ委譲する", async () => {
    ...
    expect(proxyToAxisAdmin).toHaveBeenCalledWith(request, "/api/admin/axis-definitions/surface_q");
    expect(response).toBe(sentinelResponse);
  });

  it("DELETEはaxisIdをpathへ埋め込みproxyToAxisAdminへ委譲する", async () => {
