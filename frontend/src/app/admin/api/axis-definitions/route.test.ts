...
// 軸CRUD管理APIのroute handler（改善計画T331）。proxyToAxisAdmin自体の挙動（Basic認証
// ヘッダ組み立て・転送・エラーハンドリング）はlib/adminApiProxy.test.tsで検証済みのため、
// ここではroute handlerがGET/POSTそれぞれをproxyToAxisAdminへ正しいbackendPath
...
describe("GET/POST /admin/api/axis-definitions", () => {
  it("GETはproxyToAxisAdminへ/api/admin/axis-definitionsで委譲する", async () => {
    ...
    expect(proxyToAxisAdmin).toHaveBeenCalledWith(request, "/api/admin/axis-definitions");
    expect(response).toBe(sentinelResponse);
  });

  it("POSTはproxyToAxisAdminへ/api/admin/axis-definitionsで委譲する", async () => {
