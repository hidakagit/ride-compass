...
**派生物の世代は元データの世代に従属させる**。`TILE_SCORE_MATRIX_CACHE_VERSION`は
`f"{TILE_MATERIALS_CACHE_VERSION}-{_SCORE_MATRIX_REVISION}"`という複合で、材料世代を
上げればスコア行列側も機械的に無効化される。独立した2つの文字列にすると、両方を手で
上げる運用（＝片方だけ上げる事故）が残り、「元データには在るが派生物には無いキー」が
生まれる。
