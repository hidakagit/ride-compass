...
| `tile_cache`のうち土地被覆タイル（PNG） | 関東本土を全ズーム（z6〜z14）描き切って45MB・21,264枚（2026-09-16実測、1枚あたりz14で1.7KB・z9で14KB。実際は要求されたタイルだけなのでこれは上限） | ○ 同上。ラスタ構成ごとにディレクトリが割れるため、構成を変えるとこの上限ぶんが世代ごとに積む | 手動のみ。上限が小さく実害無し（構成の変更は年次の差し替え程度の頻度） |
