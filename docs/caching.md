...
**backendは単一プロセスで動いている**（`uvicorn app.main:app`、`--workers`指定なし。
APSchedulerのバッチもリクエスト処理も同じプロセス内）。そのためRedis最大の利点である
「プロセス間で共有できる」は現状働いていない。**Redisが提供している価値は、実質
「プロセス再起動をまたいでキャッシュを温かく保つ」1点である。**

したがってRedisへ持つのは、次を説明できるものに限る。

1. プロセス内では足りない理由（＝**再起動をまたいで温かいことに実測できる価値がある**、
   または将来の複数プロセス化が具体的に見えている）
2. TTLを付けられること（`maxmemory-policy`が`volatile-lru`のため、TTLの無いキーは退避
   対象にならず、上限到達時に書き込みが落ちる）
3. 値が小さいこと（実体は常にRAMに載り、11GBのVMをPostgreSQLと共有している）

1を説明できないものはプロセス内（`cachetools`）で持つ。

**Redis自体を撤去する方針が決まっている（[T651](tasks/T651.md)、実装は[T652](tasks/T652.md)）。**
本番実測でRedisがルート生成へ与える寄与は0.2〜0.3秒（全体の2%）だった。移行が終わるまでは
既存のRedisキャッシュが動いているが、**新しいキャッシュをRedisへ追加しない**。プロセス内
（`cachetools`）かディスクを選ぶこと。
