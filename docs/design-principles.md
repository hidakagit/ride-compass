...
11. **軸は単体で評価できる**: ある軸についての良し悪しは、その軸が出す値だけで判断
    ...
    （折れ点を通す前の値）を単位付きで添える（`domain/axis_display.py: raw_value_unit`、
...
- `windPenalty.ts`がbackendの物理式をJS移植・`WIND_AXIS_THRESHOLDS`が軸スタジオから
  ...
  [T473](records/tasks/T473.md)で、`windAxisPenalties`/`gradientAxisValues`の軸ごとの別名propは
  [T483](records/tasks/T483.md)（`dedicatedWayValues`という汎用Mapへ統合）で解消済み。
...
- 面塗り撤去の調査中、`windAxisLayer.ts`の`WIND_AXIS_THRESHOLDS`（原則1違反として一度は
  ...
  override`が正しく設定されていても反映されない不具合が発覚した（[T587](records/tasks/T587.md)、
