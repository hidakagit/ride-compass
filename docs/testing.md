...
（[T834](tasks/T834.md)・[T835](tasks/T835.md)）。

**あわせて、masterのCIが赤いままなら手元へ出る**（`scripts/pre-push.sh`→
`scripts/check_master_ci.py`、[T835](tasks/T835.md)）。pushの直前にmasterの最新コミットに
対する結論を読み、成功していないワークフローがあれば警告する。**pushは止めない**——赤の原因が
自分の変更とは限らず、止めると無関係な作業がブロックされるため、気づかせるところまでを担う。
上の「1回通す」はこれで置き換わるものではない。赤を**作らない**のが手元の実行、作ってしまった
赤に**気づく**のがこの警告で、役割が違う。
...
- 制約を足した結果としてテストが大量に落ちたら、**テストの前提が本番と食い違っていた証拠**
  として読む（[T956](tasks/T956.md)ではFK追加で45件が落ち、すべて親を入れていなかった）。
