...
- 発端: 統合レビュー2026-08-22 改訂-5。T194の根本原因調査で特定済みの事項の起票
  ...
  502を返す設計で、非対称。docs/conventions/logging.mdの「エラー・429は常時可視」方針にも反する。
...
（本体の同名節にあった背景と実施順序。2026-09-21に本体を台帳へ戻す際に移した。
トリガー待ちのまま残ったT206〜T208は自分のタスク記録に背景を持つ。）

ユーザー要望「動的データの対応追加を進めたい。雷等の改善計画未記載の物含めて、取り込むべき
データを整理して」を受け、既存の動的要素導入（T170〜T178）以降に検討されていなかった
候補を実機調査つきで棚卸しした。T171実装メモが「雷ナウキャストは別レイヤー・
別調査として残す」としていた宿題も含む。設計判断はT170〜T178と同じ方針を踏襲する
...
- **Phase A（最小コスト・即着手、完了）**: T204（雷ナウキャスト、竜巻はオプション同梱）。
  ...
  Playwright実機確認で確定済みのため、残る作業は実装のみ。
...
- **Phase C（既起票の優先順位再確認、内容変更なし）**: T176（河川敷冠水、台風期の当時が
