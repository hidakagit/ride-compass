...
- 対応:
  ...
  - `docs/conventions/logging.md`に`warned`フラグの使い方（`result`+`warned`+`error_type`の3点セット）を
    追記（CLAUDE.mdが「ログ方針はdocs/conventions/logging.mdに従う」と明記しているにもかかわらず未記載
    だったため）。
...
- 完了条件: backend 786件全green（新規2件含む）。frontend側は今回の対象外
  （`debug_log.py`/`region_service.py`はバックエンドのみ、`docs/conventions/logging.md`はドキュメント）。
