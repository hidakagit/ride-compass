...
- `docs/records/decisions/pre-static-attributes-gate.md` を新規作成し、以下3点を決定する
  （ADRはドラフト→ユーザー承認で確定）:
...
この4節を「検証の自動化（Phase 1）→ 境界の固定（Phase 2）→ 内部の再配置（Phase 2後半）→
スケール準備（Phase 3）」の順に並べたのは、各修正が前の成果を安全網として使え、後続の
修正量が最小になるため（本体の「進め方の原則」節にあった順序の根拠。2026-09-21に本体を
台帳へ戻す際、対象のPhaseが全てここにあるためこちらへ移した）。
...
- `docs/architecture.md` → 現状仕様のみの `docs/architecture.md`（大幅スリム化）と、
  経緯・決定記録の `docs/records/decisions/`（ADR形式、既存の時系列記述をほぼそのまま移設）へ分割。
...
- **2026-08-23追記: トリガーが実質成立した。** T218a（gradientの探索コスト組み込み、
  ...
  詳細は`docs/records/decisions/t12-routing-scale.md` Stage 0.5参照。
...
- 完了記録: `docs/records/decisions/t12-routing-scale.md` として2部構成で確定（2026-08-23全項目承認）。
  ...
  - **2026-08-23追記（ユーザーの再検証要求を受けた訂正）**: 軸の可換性（原則7）は
    ...
    T218a（新設、トリガー: T10完了）へ切り出した。詳細は`docs/records/decisions/
