...
- （調査のみ）— トリガー: T127（全国データ取込）の意思決定時に容量試算へ含める

## クローズ（[T1028](T1028.md)、2026-09-23）

`road_nodes`表のORM定義が無く（綴りはコード内コメント2件と`batch-pipeline-dependencies.md`に
残るだけで、それらは[T997](T997.md)の範囲）、`road_edges`のPKは
`(osm_way_id bigint, segment_index smallint)`、端点も`osm_node_id bigint`。text型PKという
前提が無い。
