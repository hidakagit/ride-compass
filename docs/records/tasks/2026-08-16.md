...
- 完了条件: dry-run→実行で従来と同一件数が書き込まれること。バッチのログ
  （docs/conventions/logging.md準拠の1行INFOサマリ）へ書き込み所要時間を追加。
...
- 対応方針（候補、実装時にログで実態を見て判断）:
  ...
  - 案b: 主対象wayごとの個別envelope集合（ST_Collect）で近傍検索する（過剰包含が最小に
    なるが、クエリが複雑化）。
  - まず現状把握として、extentがbboxの何倍まで広がっているかをdocs/conventions/logging.md準拠の
...
- 問題2（0件ガード）: パーサが0件を返した場合もDELETEだけ実行され、status=succeeded・
  ...
  docs/conventions/logging.mdの「候補0件はWARNING以上・原因内訳を同行に」規約に違反。
...
- candidates=0/matched=0でも「マッチング完了」のINFO固定で、docs/conventions/logging.mdの
