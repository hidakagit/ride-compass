...
内にも定義の無い`var(--x)`参照をpre-commitでブロックする（[T675](tasks/T675.md)）。
...
個別に44pxを持つ方針へ転換した**（例: `MapLayersPanel.module.css`の`.layerTitle`
——各レイヤーの開閉見出し、パネル内で最も繰り返しタップされる操作——が
`@media (max-width: 640px) { min-height: 44px; }`を自前で持つ）。それ以外の補助的な
