...
| **過去のバグを刺した回帰** | docstringが`回帰:`で始まっている（印の無いものは下の歯止めの手順で調べる） | 残す |
...
**回帰は構造から見分けられない。** 「同じ要素を別の入力で見ている」形は重複と完全に
同じ見た目で、違うのは**なぜその入力か**だけである。それは実装に無い事実なので、
docstringが唯一の担い手になる。したがって:

- **書く人は、docstringの1行目を`回帰:`で始める。**

  ```python
  def test_a_segment_of_zero_length_does_not_stop_the_search():
      """回帰: この座標でNaNが出て、A*の優先度比較が静かに壊れた。"""
  ```

  **文面ではなく、この印で判定する。** 「この入力で落ちた」と自然文で書くだけでは、
  次に整理する人はそれを経緯コメントとして消す（[comments.md](comments.md)の既定は削除）。
  印は`grep -rn '回帰:' backend/tests`で棚卸しでき、消す側は目で読まずに除外できる。
- **消す人は、印の無い具体値だけを判断の対象にする。** 消す前に
  `git log -S'<その具体値>' -- <そのテストファイル>`で引き、当たったコミットの表題が
  バグ修正なら回帰なので**印を付けて残す**。リファクタリングなら重複である。
- 由来がどこにも無ければ重複として扱ってよい。**ただしそう判断した旨をコミット
  メッセージへ書く**——次に同じ入力が必要になったとき、探す先が残る。
...
| **過去のバグを刺した回帰** | 「この入力で落ちた」という事実。実装は「いま何をするか」しか語らない | docstringの1行目を`回帰:`で始め、何が壊れたかを書く（印が無いと次の整理で重複として消える） |
