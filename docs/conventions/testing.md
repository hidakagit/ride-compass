...
1. **既存テストを開かずに**、対象の実装だけを読み、その実装が決めていることを箇条書きで
   ...
   なくモジュールが持つデータの不変条件も数える。比較（`<`・`>=`）は境界ちょうどの値も両側に
   数え、画面部品では子の部品へ何を渡すか（どの一覧・どの倍率を渡すか）もその部品が決めている
   ことに数える。** **`if`にも分岐カバレッジにも現れない判断も数える**——`and`/`or`の各項・
   numpyのベクトル化された条件（`np.where`・`np.clip`等）・SQL文字列の中の条件・`dict.get`の
   既定値・剰余や`min`/`max`での寄せ・`Field(ge=…)`の宣言した範囲・ルーターの宣言（認証・
   `response_model`）等。ASTの`if`とカバレッジの分岐数を2通りに並べても、この形はどちらにも出ない。
...
要求する型だけ。** それ以外——他モジュールの**関数・サービス・例外・定数**——は対象の
名前空間経由で触る。
