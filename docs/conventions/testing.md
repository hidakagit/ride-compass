...
聞くと決めたら、次の順で書く。**読み手が「で、何を選ぶのか」へ辿り着くまでの距離が短くなる順**
である。

**① 利用者への約束を1文で。** 何を守るための話かを最初に置く。**製品の言葉で書く**
——関数名・引数名・「配列」「列」といった実装の語を使わない。

**② 問題が顕在化する場面を1つ。** 抽象論にしない。利用者が何をして、何が表示され、実際は
何が違うか。**固有名詞を使わない**（「環八」ではなく「片側2車線の幹線道路」——どこに住む
読み手にも同じ絵が浮かぶ語を選ぶ）。**起きない場面も1行添える**——書かないと、読み手が
自分で境界を探しに行く。

**③ 設計書・コード・テストが、それぞれ何を主張しているか。** 3行の表で、**同じ粒度・実装の語
を使わずに**。「コードはこう実装されている」ではなく「コードはこう主張している」。食い違いが
1対2なのか2対1なのかが、この表で見えるようにする。

**④ 選択肢は2つ。消すか、残すか。**

- **自分の領分を選択肢に入れない**（例外の出し方・書き方・どう直すか）
- **選択肢の文面と、実際に当てる作業を一致させる**。書く前に、その作業が何を変えるかを確かめる
- 添えるのは**消したとき外で何が起きるか**と**その道を通る呼び出し元の件数**だけ
...
実測: この形は8往復かけて直された結果である。外したのは順に——選択肢の文面と実際の作業が
違う（「型で通らなくする」と書いたが型注釈は既に済んでおり何も変わらなかった）／「消す」を
2つ並べる（違いは例外の出し方で、自分が決めることだった）／誰が決めるべきかを示さずに聞く／
軸を示した直後に承認を取らず進める／3者の主張を自分の要約で書く／設問を抽象語で書く／
「データ」「待ち」が何を指すか曖昧／実装の語で書く／固有名詞を使う。
