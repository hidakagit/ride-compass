...
## そのテストは要るか（既存テストを見直すとき）

3つを順に問う。**1つでもNoなら削除か移設**で、「念のため残す」はしない。

1. **実装のロジックに届いているか。** フレームワークの設定・結線・宣言の値を確かめている
   だけなら、守っているのは自分たちのコードではない。
2. **他のテストと重複していないか。** 「ルーターが登録されている」は、そのスイートにある
   全てのエンドポイントテストが既に証明している。
3. **このファイルの責務か。** テスト対象がファイル名の指すモジュールでないなら、移す。
4. **落ちるとき、何が壊れるか言えるか。** 言えないなら守っていない。
   「この定数がこの値であること」には答えが無く、「この境界を1つずらすと、まだ明るい時間帯に
   街灯の軸が効き始める」には答えがある。**この問いに答えられない検査は、カバレッジを
   満たすために書かれている。**

実例: `main.py`は168行のうち固有のロジックが13行（例外を握ってWARNINGへ回すデコレータ）
しかない結線のファイルだが、テストは145行10件あった。実ロジックへ届いていたのは1件で、
CORSの3件とログレベル1件はフレームワークの設定の確認、1件は他の全エンドポイントテストとの
重複だった。
