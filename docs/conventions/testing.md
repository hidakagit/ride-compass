...
`e2e/all-states.spec.ts`が、幅 × モード × 段階 × 開いているシート・パネルの全状態へ
`e2e/scans.ts`の走査を当てる。状態は`e2e/states.ts`が画面から導く:

- 幅: 幅の分岐はCSSの`--breakpoint-mobile`だけなので、その両側に1つずつ置く（両側に
  あることは実行時に検算する）。
- モード: ヘッダーのメニューに並ぶチェックボックス（研究モード等）。
- 段階: ルートの生成前と生成後。
- シート・パネル: 画面がARIAで宣言している切り替え部品——`aria-expanded`を持つ部品
  （折りたたみ・ポップオーバー・チップのグループ等）、`role="tab"`、モバイルの
  「パネル切り替え」の中の`aria-pressed`の部品。基本の状態から2段まで開く。
