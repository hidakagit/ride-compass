...
テストスイートは、1件あたりの準備コストが件数に比例して積み上がる規模にある。
...
2. **外部影響があるか。判定は「落ちるとき、何が壊れるか言えるか」。** その答えは
   ...
   - 答えられることと、それをdocstringへ書くことは別である。**答えを実装のコメントが
     ...
     「テストのdocstringも対象」）。
3. **他に実装すべき場所があるか。** その影響の持ち主が別のモジュールなら、判定はそちらの
   責務である（テスト対象がファイル名の指すモジュールでないなら、それだけで移す理由になる）。
   - **ある → 移す。** 移した先で同じことを見ているなら、移さず消す。
   - **無い → 残す。** 他のテストと似ていても残す——その入力でしか外へ出ない影響を、
     他の誰も見ていない。
...
**本番のデータそのものに対する不変条件**（全材料が説明文を持つ・参考点が有限である等）は
2問目・3問目を通る。置き場はそのデータを宣言しているモジュールのテストで、**差し替えない
節を分けて**置く（実例: `test_material_catalog.py`の`TestAgainstTheRealCatalog`）。
差し替える節と混ぜると、ファイルの責務の宣言が嘘になる。
