...
| **過去のバグを刺した回帰** | 同じ要素を、**過去に実際に落ちた具体値**で見ている | 残す。下の歯止めを読む |
...
- **回帰テストを書く人は、docstringへ由来を1行書く**（「この座標で落ちた」「この桁で
  0になった」）。書かなければ、次に整理する人には重複としか見えない。
- **消す人は、由来の無い具体値を消す前にgit履歴を引く**
  （`git log -S'<その具体値>' -- <そのテストファイル>`）。当たったコミットの表題が
  バグ修正なら回帰で、リファクタリングなら重複である。
- 由来がどこにも無ければ、重複として扱ってよい。**ただしそう判断した旨をコミット
...
| **過去のバグを刺した回帰** | 「この入力で落ちた」という事実。実装は「いま何をするか」しか語らない | 残すときは、なぜ残すかをdocstringへ1行書く |
