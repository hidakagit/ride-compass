...
4. **落ちるとき、何が壊れるか言えるか。** 言えないなら守っていない。
   ...
   満たすために書かれている。**
   **ただし、その答えを実装のコメントが既に持っているなら、docstringへ書き写さない**
   ——テストのdocstringに書いてよいものの判定基準は
   [comments.md](comments.md)「テストのdocstringも対象」が持つ。
...
| **上のどれとも言い切れない** | 同じ要素の別の入力に見えるが、重複と断じる根拠が無い（過去のバグを刺した回帰がここへ来る） | 下の3問を通す |
...
分けられないので、**1件ずつ問う**。上の表で重複・責務外に見えたものも、消す前にこれを通す。

1. **外部影響があるか。** そのテストが守っているものは、このモジュールの外——利用者の
   画面・他モジュール・配信される値・保存されるデータ——へ出るか。
   - **出ない → 消す。** 内側で閉じた言い換えは、何も守っていない。
2. **他に実装すべき場所があるか。** その影響の持ち主が別のモジュールなら、判定はそちらの
   責務である。
   - **ある → 移す。** 移した先で同じことを見ているなら、移さず消す。
   - **無い → 残す。** 重複に見えても残す——その入力でしか外へ出ない影響を、他の誰も
     見ていない。

**上の2問は、起こし直し版に元からある検査にしか使えない。** 旧版にしか無いものを
「他に場所が無いから残す」と言って戻すのは、4分類を飛ばして持ち込むことである。
**起こし直した版に無い要素は、旧版から戻さない**——`git log -S`で初出を引き、自分の
起こし直しのコミットなら自分の誤削除なので戻してよく、それ以前なら列挙漏れとして
**列挙をやり直し、実装から自分で導く**。逐語で戻したかどうかは関係がない（同じ要素を
言い換えずに持ち込んだだけである）。
...
判定と理由はコミットメッセージへ書く。**「重複だから消した」で止めず、上の2問のどちらで
...
判定の問いは1つ——**実装を読んで復元できるか**。できないものだけが残す対象になる。
...
| **意図的に外した値と、その理由** | 「足しても1件も落ちない」ものを、落ちるようにしてある | `barrier`から`kerb`を外す理由（該当件数は多いが段差の過半がlowered/flushで識別力が無く、足すと停止密度が跳ね上がる）。当事者種別59（軽車両－その他）は自転車ではない |
| **過去のバグを刺した回帰** | 「この入力で落ちた」という事実。実装は「いま何をするか」しか語らない | 由来はコミットメッセージが持つ（テストのdocstringへ経緯は書かない）。消すかどうかは上の3問で決める |
