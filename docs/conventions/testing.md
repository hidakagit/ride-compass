...
**要確認を出すときは、選択肢を先に置く。** 判定してほしいのは「消すか、残すか」の一点で、
背景を先に積むと、答える側がそれを探す作業をすることになる。1件につき示すのは次の3つだけ:

- **消すか、残すか**（他の選択肢があるならそれも1行で）
- **消したとき何が起きるか**——本番の挙動が変わるなら必ず書く
- **その分岐を通る呼び出し元が実在するか**、件数で

docstringの引用・実装の経緯・自分がどう判断したかは、聞かれてから出す。
**3つ目を落とすと、材料の欠けた判定になる**（実測: `edge_seconds`を必須にしてよいかを
「今は無害」とだけ言って尋ね、省略している呼び出し元が1件実在することを伝えていなかった。
判定を差し戻して聞き直した）。
