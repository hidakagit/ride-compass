...
**新しく書くとき・既存を見直すとき・旧版から取り込むか決めるときは、すべてこの3問を使う。**
上の問いで決まったらそこで終わりで、「念のため残す」はしない。
...
3. **カバレッジで穴を塞ぐ**。対象モジュールへ分岐カバレッジを当て、届いていない行・分岐を
   出す。**母集団は「同名のテストファイル」ではない**——テストは振る舞いで名付けられる
...
5. 両方向を判定する。
   ...
   - **旧版にしか無い**: 他ファイルの責務なら**移す**、下の「残すべきテストの型」に当たるなら
     **残す**、それ以外は**消す**。
...
分けられないので、**上の表で「言い切れない」へ落ちたものは上の
[そのテストは要るか](#そのテストは要るか3問を順に)の3問を通す**。回帰はそこで3問目の
「他に実装すべき場所が無い」として残る。

**3問は、起こし直し版に元からある検査と、上の表で「言い切れない」へ落ちたものにだけ
使える。** 旧版にしか無いものを「他に場所が無いから残す」と言って戻すのは、4分類を
