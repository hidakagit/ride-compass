...
- **訂正（2026-08-31、[T456](T456.md)）**: T396/T397（2026-08-30、4テンプレート→
  2プリミティブへの再設計）により`ShapeKind`型（`AxisComposer.tsx:33`）が4種から
  `flag_sum`撤去で3種（`breakpoint_linear`/`recipe_then_breakpoint_linear`/
  `categorical`）へ減った。上記「5つ目のshape種到達」は当時の4種を前提にした
  「+1種で発火」という意図だったため、母数が変わった今は**「4つ目のshape種到達」**が
  同じ意図（+1種）に相当する閾値として正しい。現在有効な閾値はこちらへ読み替える
  （行数閾値の1,400行は変更なし）。
