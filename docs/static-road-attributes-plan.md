...
### 2.5 未評価の新規候補（2026-08-17追記・実測済み）

2026-08-17のユーザー依頼（地図に未表示のOSM情報を実データ充当率込みで再ランク付け）で新たに
挙がった、本計画の棚卸し（§2.1/2.2/2.3）にまだ無いタグ。**[improvement-plan.md](improvement-plan.md)
T102として実測完了（2026-08-17、関東全域PBF `kanto-latest.osm.pbf`、対象way 1,329,632件）**。

| 属性 | OSMタグ | 取得元 | 想定用途 | カバレッジ実測 | 判断 |
|---|---|---|---|---|---|
| 街灯 | `lit=*` | way | 早朝/夜間走行時の安全性判断 | 全体1.1%（幹線4.8%・生活道路0.5%・自転車歩行者道等5.5%） | **採用推奨**（既採用のsmoothness実測0.2%より高く、幹線道路に偏っており早朝夜間の安全性判断という用途に合致） |
| 歩車分離 | `segregated=yes/no` | way | 自転車歩行者道（§2.1「歩行者共用」T99）の歩車分離有無、分類の粒度向上 | 全体0.6%だが、T99で新規取込した自転車歩行者道（highway=footway/path AND bicycle可）に限ると**28.4%**（同PBF内、他グループとの差が大きい） | **採用推奨**（全体では低いが、まさに使いたい対象＝自転車歩行者道内では約3割に付与されており、`shared_pedestrian`分類の粒度向上という用途に直接合致。タグ自体は既にT99でALLOWED_WAY_TAGSへ追加済み、`classify_bicycle_infrastructure`への反映は別タスクで検討） |
| 車止め等の障害物 | `barrier=gate/bollard` | node | 河川敷サイクリングロード等での通行可否判定 | 母数の分母が無いため件数のみ: bollard 21,975件・kerb 9,488件・gate 8,535件・cycle_barrier 3,980件・toll_booth 1,271件（以下ロングテール） | **採用推奨（ただし別スコープ）**。件数は明確に意味のある規模。ただしway属性のlit/segregatedと異なり現在の取込プロファイルに対応するnodeルールが無く、新規node取込ルール＋`osm_raw_pois`への追加（既存のstop_inducing/踏切と同じ機構）が必要なため、採用する場合は新規タスクとして起票する（T102の範囲は実測・判断までで実装は含まない） |

**実測値の脚注**: `smoothness`（既採用済み、道路種別基準の判定に使用）は本計測で0.2%、
`width`/`shoulder`（P2据え置き確定済み）は0.4%/0.0%だった（初回実測時点の0.1%/0.3%/0.0%から
微増、T99でfootway/pathの一部が新たに母集団へ加わったことによる分母変化が主因、傾向に変化なし）。
