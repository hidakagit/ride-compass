...
### - [ ] T241. 道路グラフの連結性調査（弱連結成分の分断でルート探索が失敗するケース） 規模S〜M（調査のみ）
...
次点4件（T226〜T228・T231/T232）が本セッションで全て完了したため全面改訂した。
第4版以前の本文はgit履歴（本ファイルの各棚卸コミット）を参照。残るのは指示待ちの
着手可能タスクとトリガー未到達タスクのみで、いずれも急ぎではない。
...
- **今すぐ着手できる（トリガー成立済み・指示待ち）**:
  ...
  4. **T241**（道路グラフの連結性調査、規模S〜M・調査のみ）: T240の実データ検証中に
     発見（2026-08-23起票）。トリガー条件なし、指示待ち。
...
**サマリ（12タスク）**: 指示待ち4件（T209・T221・T223・T241）／
