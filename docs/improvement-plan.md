...
### - [ ] T293. 周回ルートの採用向き（順回り/逆回り）を地図上へ矢印で明示する 規模S〜M — トリガー解消（2026-08-25、別セッションのT292フロント移行が完了・本番反映済み）、着手可能
...
2026-08-25にT292フロント移行完了でトリガー解消、着手可能）・
**T295〜T298（全体最適レビュー第9回の起票4件、いずれもトリガー無しの即時着手可能項目。
T295が最優先）**が着手待ち。T275（Tailwind採否）は
