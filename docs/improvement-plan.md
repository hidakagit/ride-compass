...
## 残タスクの優先順位（2026-08-23再整理・第10版）

第9版の最優先事項だった「`routing_engine`既定値切替の最終確認」は同日中にユーザー判断
（「既定値を切り替えして」）により決着し、T247として実施済み。**T236〜T247を通じて
ORS→road_graphエンジンの本番化に関する残タスクは全て完了した**。第10版として更新した。
...
- **参考記録（対応は不要〜任意、監視のみ）**:
  - `node_upsert_ms`/`edge_upsert_ms`（バルクUPSERT）が現在の支配的コスト（王子30kmの
    実測で計約195秒）。初回タッチ時のみのコストのため優先度は低いが、今後さらに広域の
    リクエストで問題化する場合は別途調査対象とする。
  - T241で見つかった一部方位での「経路が見つからない」事象（8方位中平均1〜2方位）は
    残存するが、実運用への影響は限定的と評価済み（T241本文参照）。

- **トリガー未到達（現時点では着手しない）**:
  3. **T206**（積雪・凍結、規模S〜M）: 冬季前=11月。季節トリガーの到達が最も近い。
  4. **T145a**（night軸レイヤー、規模S〜M）: litタグデータの充実待ち（変化なし）。
  5. **T105**（バックエンド到達不能の原因特定、規模S〜M）: 次回の再現報告待ち。
  6. **T127**（全国データ取込の検証、規模不明）: 全国展開の意思決定待ち。
  7. **T145**（レイヤーパネルのレジストリ駆動化・三次レイヤー、規模L）: 裁量待ち。
  8. **T207**（CAPE延長予報、規模S）: 利用実績・要望待ち。
  9. **T208**（視程・霧調査、規模S）: 利用報告待ち。
  10. **T229**（冷パス計測、規模S）: 体感遅延報告待ち（T224でのエンドツーエンド実測は
...
T209・T223・T241は調査完了・T242〜T247は実装完了（road_graph本番化は改善効果・
既定値切替とも実データ検証・反映済み）のため本リストから除外した。
