...
- [ ] [T1091](records/tasks/T1091.md). バッチの入口（取込・派生・DBの立ち上げ）が1つずつ持つ起動処理の写しを共有の関数へ寄せる 規模S
- [ ] [T1092](records/tasks/T1092.md). ルートの割増の強さ（P）の既定と、軸ごとの寄与度の計算を、それぞれ1つにする（地点確認のAPIもルート生成と同じPで探す） 規模S
- [ ] [T1093](records/tasks/T1093.md). 道路タイルに焼く真偽の材料（照明・トンネル・自転車レーン等）の条件を、材料の宣言から組み立てる 規模S
