...
## 残タスクの優先順位（2026-08-24再整理・第15版）

第14版以降、T221 Part 3（Stage D: 評価軸レジストリのDBテーブル化＋管理API）を
ユーザー指示「T221のDに進んで」を受けて実施した。未決だった3点の製品判断（GUI編集の
開放範囲・極端な重み設定への歯止め・T12 Part 2キャッシュ無効化条件との整合）を確認し
それぞれ確定、加えて将来の権限制御導入方針を踏まえ管理APIの認可を差し替え可能な設計に
した（T221エントリの「Part 3実装メモ」参照）。Stage E（GUI編集画面）はADR自身が
「別タスクとして起票する」とスコープ外にしているため、着手判断があった時点で新規タスク
として起票する（本リストには載せない）。第15版として結果を反映した。
...
BottomSheetは断念で完了扱い・T229はT248へ
統合クローズ・T221はPart 3（Stage D）まで実装完了（Stage Eは別タスクとして起票予定）のため
