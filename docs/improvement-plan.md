...
- [ ] [T1120](records/tasks/T1120.md). 道の線の「不明」「その他」を、地図の上で見える濃さで描く（不透明度0.15→0.6） 規模S
- [ ] [T1121](records/tasks/T1121.md). 配信で値を受け取る評価軸（勾配のレンズ等）の線が、値があっても全部破線になるのを直し、地図の式がfeature-stateを読めない場所で使っていないかを検査する 規模S
- [ ] [T1122](records/tasks/T1122.md). 道路の種類の線の太さの差をやめ、道の線をすべて同じ太さ・固定の間隔に戻す（T1110の取り消し） 規模S
- [ ] [T1123](records/tasks/T1123.md). ルート結果パネルで、候補一覧だけを縦スクロールできるようにし、操作ボタンを低くし、最速の候補は所要時間と印で示す 規模S
- [ ] [T1124](records/tasks/T1124.md). ルート結果に、道のりに沿った難易度のグラフ（横軸が距離、縦が区間ごとの難易度、塗った面積が負荷）を出す 規模M
...
- [ ] [T1117](records/tasks/T1117.md). データが欠けたとき黙って既定値で計算している箇所を洗い出し、利用者に知らせるか断るかを決める 規模S
