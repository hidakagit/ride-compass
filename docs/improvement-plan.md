...
- 完了条件: 表示項目の設定パネルで非表示に選んだメンバー/軸が、グループ展開時に
  実際に除外されること。設定を変えていない他のメンバー/軸のON/OFF・凡例展開が
  T128までの挙動と変わらないこと。モバイル幅でのPlaywright実機確認。
- 検証: `MapOverlayControls.test.tsx`（既存1件更新・新規1件追加、計30件green）。
  frontend 492件・tsc/eslint全green。Playwrightでモバイル幅（390px）の実機確認:
...
- **トリガー未到達（現時点では着手しない）**: T206（積雪・凍結、冬季前=11月）・
  ...
  T181（レイヤー見切れの実際の報告待ち）・T203（モバイル実機での遅さ報告待ち）。
  いずれもトリガー未到達の実装を「ついで」にやらない（設計原則10）。
