...
修正した。修正後の再実測はT243完了をもって「根本原因の解消」を優先し今回は実施していない
（必要なら別途実施可能）。連結性（弱連結成分）の解析結果はT241へ統合済み。
...
## 残タスクの優先順位（2026-08-23再整理・第6版）

第5版時点で指示待ちだったT209（黄砂・PM2.5調査）・T223（DEM1A調査）は本セッションで
完了（いずれも調査のみ、結論は本文参照）。**ユーザー指示によりORS→road_graphエンジンの
本番化を最優先事項へ引き上げ**、第6版として全面改訂した。

- **最優先（ユーザー指示: road_graphエンジンの本番化）**:
  1. T243（DBタイムアウト分離）適用後の**本番DBでの性能・成功率の再実測**。今回の実測
     （本文「本番DBでの性能・成功率実測」参照）はT243修正前のコードで行ったため、
     30km級3パターンが全てTimeoutErrorで欠測のまま。修正後の実測値を得て、T224の
     「5秒以内」目標への到達状況を本番データで確認する。
  2. 上記の実測結果を踏まえ、`config.py: routing_engine`の既定値切替（現状
...
T209・T223・T241は調査完了・T242〜T244は実装完了のため本リストから除外した。
