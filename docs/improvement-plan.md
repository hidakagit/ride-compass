...
- [ ] [T1098](records/tasks/T1098.md). 管理APIの口ごとの「認証なしで401」のテストを、全ルート走査（test_admin_route_authorization.py）へ寄せて消す 規模S
- [ ] [T1099](records/tasks/T1099.md). 風の詳細格子の間隔を「キャッシュ共有のため離散的な段階だけ」とする注記が今も成り立つかを確かめ、成り立たなければ本当の理由へ直す 規模S
