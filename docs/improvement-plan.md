...
- [ ] [T1112](records/tasks/T1112.md). 本番backendが、都心の40〜80km程度の周回の初回の生成（探索範囲に約270万Edge）でメモリ上限6GBを超えて落ちる 規模L
...
- [ ] [T1110](records/tasks/T1110.md). 順序のある分類（道路種別）の線を、色と同じ順序で太さでも示す（太さはbackendが順位から導く） 規模M
...
- [ ] [T1114](records/tasks/T1114.md). テスト規約に「確かめる高さ」を定め、カバレッジを「下げない」から「下げたら理由を書く」へ改める 規模S
- [ ] [T1115](records/tasks/T1115.md). 勾配の材料名の直書きをやめ、各所に書き写された同じ計算を1か所にまとめる 規模M
- [ ] [T1116](records/tasks/T1116.md). 内部の構造に強く依存しているテスト3本を、公開の入口で確かめる形へ置き換える 規模S
