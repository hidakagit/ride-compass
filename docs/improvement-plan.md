...
横断して仕分けた。判断基準は依存関係・季節等のタイミング制約・コストの3軸
（Phase順の考え方はT170〜T178・T204〜T211の整理と同じ）。

- **すぐ着手できる（トリガー無し、優先度順）**:
  1. **T174**（夏季WBGT警告、規模S〜M、2026-08-22完了）: 提供期間が4〜10月で、今
     （8月）はまさに提供期間の中盤。T205（警報・注意報バッジ）で共有表示コンポーネント
     （`WarningBadge.tsx`）を作ったばかりのため実装コストが下がっており、
     シーズン終了前に間に合わせる価値が最も高い。
  2. **T176**（河川敷冠水の調査、規模S〜M、2026-08-22完了）: 調査の結果、当初想定の
     「川の防災情報」（機械アクセス禁止で不採用）ではなくJMA指定河川洪水予報APIという
     代替を発見し「実装推奨」の結論に至った。実装はT212として別途起票済み。
  3. **T210**（通行止め・道路規制の調査、規模S〜M、2026-08-22完了）: 調査の結果、
     JARTIC規制情報が月次更新でリアルタイム性が無く、xROADにも該当データベースが無いため
     「見送り」の結論に至った。
  4. **T177**（交通量時間帯補正の実現性調査、規模M、2026-08-22完了）: JARTIC実測4地点の
     時間帯プロファイルが地点ごとに大きく異なり一般化できるパターンが確認できず、
     T53の観測点選択バイアスもそのまま制約になるため「見送り」と判断した。
  5. **T175**（花粉飛散情報の調査、規模S、2026-08-22完了）: Open-Meteo pollenは日本域
     非対応と実機確認、ウェザーニューズのポールンロボは無料公開だが「インターネット
     公開の場合は要問い合わせ」の条件があり無条件で組み込めないため「見送り」と判断した。
- **トリガー未到達（現時点では着手しない）**: T206（積雪・凍結、冬季前=11月）・
  T207（雷ポテンシャルCAPE延長予報、T204の利用実績待ち）・T208（視程・霧、利用報告待ち）・
  T209（黄砂・PM2.5、T175完了待ち）・T10/T11/T12（Road Graphの大規模構造変更、
  意思決定/常用化待ち）・T105（再現報告待ち）・T127（全国展開の意思決定待ち）・
  T145/T145a（三次レイヤーの実施時期はユーザー判断／night軸データ充実待ち）。
  いずれもトリガー未到達の実装を「ついで」にやらない（設計原則10）。
