...
- **未解決のまま残る点**: 本番実測で見た100秒超×チャンク規模の遅延を、合成データでは
  正確に再現できていない（実際の`new_edge_ids`がテスト規模よりさらに大きかった可能性、
  または複数の要因の複合による可能性）。今回の対応（一時テーブル化＋work_mem引き上げ）は
  いずれも設計上正しい改善だが、実際の改善幅はT245で追加したログが次回の本番アクセスで
  実際のnodes/edges/way_ids_to_replace件数とdelete_msを記録するまで確定できない。
...
## 残タスクの優先順位（2026-08-23再整理・第8版）

第7版時点の最優先項目1（save_graphのDELETE遅延の原因特定・対策）はT246として実施済み
（ロック競合を実機監視で確定的に否定、チャンクごとの除外配列再評価という真因を特定し
一時テーブル化＋work_mem引き上げ[トランザクションローカル256MB・本番グローバル16MB]で
対応）。ただし合成データでは実測の遅延幅を完全に再現できておらず、実際の改善効果は
未確定のまま。第8版として更新した。

- **最優先（ユーザー指示: road_graphエンジンの本番化）**:
  1. **T246の修正後、実際の本番トラフィック（または王子30km等の再実行）でT245の
     ステージ別ログ（`delete_ms`等）を確認し、改善幅を実データで検証する**。合成データでは
     28%短縮のみ確認済みだが、本番実測（100秒超×チャンク規模）に対する実際の効果は
     未検証。
  2. 上記の検証結果を踏まえ、`config.py: routing_engine`の既定値切替（現状
     `"openrouteservice"`）の可否をユーザーと最終確認する。切替判断そのものは
     ユーザー裁量（設計原則10に反しない意思決定事項）。
  3. T242の残課題（標高バックフィルの定期再実行、新規splitされたEdgeへの反映）を
     自動化するか都度手動実行に留めるかの方針決定。

- **指示待ち（トリガー成立済み）**:
  4. **T221**（評価軸のフルレジストリ駆動化、規模L・方向性のみ承認済み）: 指示待ち。
     着手時はStage Aの詳細設計から。T226（残骸削除）・T222（Overpassライブ経路削除、
     いずれも完了済み）で土台は既に整理済み。road_graphエンジン本番化とは独立した
     設計課題のため優先度は本番化より下。
...
- **トリガー未到達（現時点では着手しない）**:
  ...
  12. **T229**（冷パス計測、規模S）: 体感遅延報告待ち（T224でのエンドツーエンド実測は
      ...
      road_graph本番化の実測（上記1）と相乗りできる可能性がある）。

いずれもトリガー未到達の実装を「ついで」にやらない（設計原則10）。

**サマリ（12タスク）**: 最優先3件（road_graph本番化関連、上記1〜3）／指示待ち1件（T221）／
トリガー未到達8件（T206・T145a・T105・T127・T145・T207・T208・T229）。
T209・T223・T241は調査完了・T242〜T246は実装完了のため本リストから除外した
（T246は真因を特定し対応済みだが、改善効果の実データ確認は次回の本番アクセス待ち）。
