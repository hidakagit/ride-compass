...
- フォローアップ（2026-08-26、Render実機のスクリーンショットでユーザーが発見）:
  ...
  2. **「値」欄に生のOSMタグ値（例: `cycleway`）がそのまま表示され分かりにくい**という
     ...
     自体は生のタグ値を表示する`<input>`のままだった（保存されるvalue自体は生のタグ値の
     ままにする必要があり、単純に表示だけラベルへ差し替えると入力中の文字列と表示文字列が
     食い違い編集不能になるため、素朴な差し替えはできない）。ラベルが引ける行（候補から
     選んだ、または既知のタグ値を手入力した）は既定でラベルの読み取り専用表示
     （「直接入力する」ボタンで生のタグ値の編集欄へ戻せる）にし、ラベルが引けない値
     （候補に無い独自の値等）は従来どおり入力欄のままにする`manualEditCategoricalRows`
     （行indexのSet、材料切替時にリセット）を追加した。
  検証: `AxisComposer.materialValues.test.tsx`の該当テストを新しいUX（候補選択後はラベル
  表示span、「直接入力する」で生の入力欄へ戻る）に合わせて更新。フロント全680件・lint・tsc
  green。
