...
- **Part 2実装メモ（2026-08-23完了、ブランチ`t221-registry`）**:
  ...
  - **残作業（保留、影響範囲付き）**:
    1. **masterへの統合**: 並行セッションのT250（`page.tsx`等）コミット後にマージする
       （未統合の間、master側はT221の成果を含まない。`page.tsx`はT250と本タスクの
       両方が触れており、マージ時に小規模なコンフリクト解消が要る可能性がある）。
...
ユーザー指示で実施し完了した（ブランチ`t221-registry`、T221エントリの実装メモ参照）。
第12版として更新した。

- **統合待ち**:
  1. **T221 Part 2の成果のmasterへのマージ**（ブランチ`t221-registry`、3コミット）:
     並行セッションのT250（モバイル上部バーUI）コミット後に実施する。マージ後に
     研究タブ重みUIのPlaywright実機確認を行う（T221エントリの残作業参照）。
...
**サマリ（11タスク）**: 統合待ち1件（T221 Part 2成果のmasterマージ＋実機確認）／
指示待ち2件（T242残課題・T221 Stage D/E）／
トリガー未到達8件（T248・T206・T145a・T105・T127・T145・T207・T208）。
T209・T223・T241は調査完了・T242〜T247・T249は実装完了・T229はT248へ統合クローズ・
T221はPart 2（Stage B+C）まで実装完了のため本リストの指示待ちから外した。安全網の回復3件（T225・T224・T230）とレビュー指摘の
