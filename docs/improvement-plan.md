...
### - [ ] T218. 探索の素材事前計算化＋リーンロード（T12 Stage 0）規模M — トリガー: ユーザーの実装着手指示
...
## 残タスクの優先順位（2026-08-23再整理・再検証反映版）

ユーザー要望「改善計画の残タスクを優先度順に洗い出して、それぞれの実装規模を明記して」を
受け、2026-08-22整理からの差分（T12確定・T218〜T220新設・T209のトリガー成立）を反映して
洗い出した後、ユーザーからの2件の再検証要求（T10とT218の関係／評価軸の増減が本当に
自由度高く可能か）を受けて内容を訂正した版。判断基準は前回と同じ3軸
（依存関係・季節等のタイミング制約・コスト）。

**この再整理で判明した最重要事項**: T218（探索の素材事前計算化）に含めていた
gradient（勾配）の事前計算は、素朴に実装するとT10（DEMタイル化＋標高キャッシュ
1系統化）のトリガー「全道路網への標高属性の一括事前計算」をそのまま満たしてしまう
ことが実装精査で確定した（現行の標高取得は1エッジ平均6点のGSI点API呼び出しが必要で、
本番規模に外挿すると数十万〜百万オーダーの呼び出しになる）。そのためgradient対応を
T218から**T218a（新設、トリガー: T10完了）へ切り出した**。この結果、**T10は
「トリガー未到達で優先度最低」から「Part 1で確定済みの決定（gradientを探索コストに
入れる）を実現する直接の前提タスク」へ位置づけが変わっている**。

- **今すぐ着手できる（トリガー成立済み、優先度順）**:
  1. **T209**（黄砂・PM2.5情報の導入可否調査、規模S・調査のみ）: トリガー「T175完了後」が
     2026-08-22のT175完了時点で既に成立している。着手コストが全残タスク中最小の調査で、
     見送るべき理由が無い。
  2. **T218**（探索の素材事前計算化＋リーンロード、T12 Stage 0、規模M、
     **gradientを除く6素材構成に縮小**）: T12 ADR（Part 1・Part 2）が2026-08-23に
     全項目確定し設計上の障害は無い。トリガーは「ユーザーの実装着手指示」のみ——
     外部要因待ちではなく、ユーザーが号令を出せば即着手できる状態。次に着手すべき
     最有力候補だが、**本整理は洗い出しのみで着手はしない**（明示指示を待つ）。
...
- **トリガー未到達（現時点では着手しない、トリガー成立が近いと見込まれる順）**:
  ...
  5. **T10**（DEMタイル化＋標高キャッシュ1系統化、規模L）: トリガー「全道路網への標高
     属性の一括事前計算が必要になったとき」。**確定（2026-08-23再検証）**——上記の
     とおりT218a（gradientの探索組み込み）の直接の前提であることが判明したため、
     「トリガー未到達の低優先タスク」という従来の位置づけは実質的に見直しが必要。
     山岳ルート回避（ユーザー要件、2026-08-23）を実現するにはT10を経由する以外の
     現実的な道筋が無い。
  6. **T218a**（gradientの探索コスト組み込み、T12 Stage 0.5、規模M〜L、新設）:
  ...
  10. **T145**（地図レイヤーパネルのレジストリ駆動化・三次レイヤー新設、規模L）:
      ...
      持たず「実施時期はユーザー判断」という裁量待ちのまま。残タスク中もっとも
      優先度が低い。
...
**評価軸の増減の自由度について（2026-08-23再検証）**: 「7軸を8個にする等の増減は
本当に自由度高く可能か」という設問に対し、T12 ADR原則7を実装精査した結果、当初の
「レジストリ宣言＋変換関数の変更だけで完結する」という記述は不正確だったと判明し訂正した
（詳細はdocs/decisions/t12-routing-scale.md 原則7参照）。正確には:
軸の増減は**backend約10箇所＋frontend約5箇所（`RoutePreference`・`AxisDifficulties`・
軸別手書き対応表・API層モデル・`route_preference.yaml`・フロントの略名/アイコン/
レイヤー対応表等）への決まった機械的なコード変更**で完結し、**1次素材の再計算・
バックフィルは不要**。過去3回（T138・T148・T149）とも実際にこの規模で完遂できている。
「レジストリ2箇所だけ」ではないが、「1次から作り直す」よりは明確に軽い、という
中間的な結論になる。
