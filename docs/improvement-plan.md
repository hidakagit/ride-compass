...
- [ ] [T1014](records/tasks/T1014.md). E2Eを「実機でしか出ないもの」の場として作り直す 規模M〜L — トリガー: T1078の完了（その後に開発機で`npm run test:e2e`が通るかを確かめ、所要を測り直して閉じる）
...
- [ ] [T1018](records/tasks/T1018.md). 標準を「操作別」に見直す（撤去・移設の手順が無い穴を塞ぐ） 規模M — トリガー: logging.md・caching.md・documentation.mdの節を消して、そこにしか無い判断が失われた事例を1件観測したとき
...
- [ ] [T395](records/tasks/T395.md) 令和3年度道路交通センサスデータの活用可否調査〔調査のみ〕 — トリガー: 実装着手の判断がユーザーから出たとき（2026-09-23 ユーザー決定で保留を継続）
...
- [ ] [T1082](records/tasks/T1082.md). 降水短時間予報（rasrf）の配信要素と時刻一覧の在り処を源泉へ宣言する 規模M
...
- [ ] [T1057](records/tasks/T1057.md). 並行実行の重い処理を機械全体で1つの枠に並べ、CPUの取り合いを無くす 規模S（残り: 枠の数の実測・効いていることの確認） — トリガー: 司令塔が独占の時間帯（稼働中の担当が他に無い）を確保したとき
