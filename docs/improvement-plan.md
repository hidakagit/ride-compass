...
- 背景: 現状の降水情報は「降水確率」（Open-Meteo、地点値）のみで、雨雲の空間分布が
  ...
  最も高い動的表示。気象庁の高解像度降水ナウキャスト（5分毎更新・1km格子、実況＋60分先
  予測。さらに降水短時間予報で15時間先まで1時間刻み）は時刻パラメータ付きPNGタイルとして
...
- 対応方針: 基準時刻一覧（targetTimes JSON）を取得→スライダーの対象時刻に最も近い
  ...
  仕様変更検知（取得失敗をT87のerror状態へ落とす）を必ず入れる。雷ナウキャスト
  （活動度1〜4、同じタイル系）は同じ仕組みに載るため、実装時に工数差が小さければ
  同時に追加してよい（別レイヤー・警告表示扱い）。
