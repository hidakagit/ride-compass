...
## 残タスクの優先順位（2026-08-24再整理・第17版）

第16版以降、**T248（road_graph既定エンジンの性能改善）が完了した**。T259で確定した
「未split・20km規模地点がRenderの約100秒プラットフォームタイムアウトで完全失敗する」
事象は、T263（Oracle VM移行）・T264（closure_ms削減）で構造的に解消済みであることを
本番backendへの直接検証（20km/30km・複数地点）で確認し、さらに温パス5秒以内
（T224目標）未達の原因だった「タイル材料キャッシュの書き込み漏れ」も修正した
（T248本文1e参照）。冷パスの体験設計（バックグラウンドウォームアップ・事前split等の
大掛かりな実装）はユーザー判断により今は実施せず、T265へ切り出して将来課題として保留
した。T242の残課題（標高バックフィルの定期再実行）は「当面は手動実行、新規インフラは
立てない」という暫定方針で運用するが、**タスクとしては完了させず、トリガー未到達の
継続監視項目として残す**（新規splitの頻度増加・欠損起因の品質劣化報告のいずれかが
起きたら自動化を再検討、T242エントリの「暫定方針」参照）。
...
ウォームアップ等）はT265へ切り出してトリガー未到達リストへ移した。**
