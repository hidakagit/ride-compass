...
### - [ ] T261. 大規模な冷パスリクエストで本番バックエンドプロセスがクラッシュ→自動復旧する事象 規模不明（調査中、2026-08-24）
...
- 次の調査候補（未着手、ユーザー判断待ち）:
  1. Renderダッシュボードの当該時間帯（2026-08-23 22:55〜22:57 UTC付近）のログ・
     メトリクス（メモリ使用量グラフ・再起動理由）をユーザーに確認してもらう
     （本セッションからは見えない情報）。
  2. Renderのインスタンスプラン（メモリ上限）を確認し、必要なら増強を検討する。
  3. OOM説が正しい場合、`get_way_specs_with_closure`/`build_road_graph`が
     都心規模で数万〜数十万件のオブジェクトを一括でメモリに保持する設計自体が
     律速要因になる。ストリーミング処理・チャンク単位での逐次処理・より軽量な
     中間表現（T248候補1dのlean型分離と同じ方向性）等、メモリ使用量そのものを
     減らす設計変更が必要になる可能性がある。規模はL以上になりうる。
- 完了条件: 未達（原因未確定）。Renderダッシュボード側の情報が得られ次第、
  OOM説の確定または棄却を行い、対応方針を確定する。
...
- **指示待ち（ユーザー判断・情報提供待ち）**:
  1. **T261**（大規模な冷パスリクエストで本番バックエンドプロセスがクラッシュ→自動
     復旧する事象、規模不明）: 現在も再現する未解決の本番障害。Renderダッシュボードの
     メモリ使用量・再起動理由の確認をユーザーに依頼済み（本セッションからは見えない）。
     詳細は本文参照。
...
**サマリ（11タスク）**: 指示待ち1件（**T261、現在も再現する未解決の本番障害**。
T248のトリガー見直し要否も併せてユーザー判断待ち、上記参照）／トリガー未到達9件
（T248・T206・T145a・T105・T127・T145・T207・T208・T242残課題）。T209・T223・T241は
調査完了・T242本体（migration 0013適用・標高バックフィル初回実行）・T243〜T247・
T249〜T254・T256〜T259は実装/調査完了・T255はDynamicLayerTimeSlider完了（ホイール
操作は実機確認済み）・BottomSheetは断念で完了扱い・T229はT248へ統合クローズ・T221は
Part 3（Stage D）まで実装完了（Stage Eは別タスクとして起票予定）のため、いずれも
...
トリガー未到達のまま残る。T259（20km失敗の根本原因特定）は調査として完了したが、
その結果はT248のトリガー再検討という新たな指示待ち事項を生んだ。**T248候補1
（バルクUPSERTのCOPY化）は本番障害を受けて実装当日中に切り戻し済み（T261参照）**。
