...
### - [ ] T194. Open-Meteo 429による気象グリッド欠損の根本対策（6段階ロードマップ） 規模M〜L — トリガー: ユーザーが優先度を判断し着手を指示したとき
...
- **④ 3時間程度のTTLでDBキャッシュ — 部分実装**。現状はプロセス内メモリ辞書
  （`_wind_forecast_cache`、`weather_client.py:73`）でDBキャッシュではなく、再起動で消える。
  正のTTLは`CACHE_TTL_SECONDS`＝30分（同ファイル:20、「3時間程度」の要求値ではない）。
  ただしTTL切れ後も`STALE_FALLBACK_MAX_AGE_SECONDS`＝3時間以内の古いキャッシュへ
  フォールバックする仕組み（同ファイル:61）は既にあり、「3時間」という数値自体は劣化時の
  救済としてコード上に存在する。
...
- **⑥ 利用者が増えたらOpen-Meteo自前運用 — 未着手**。関連策としてT179（Oracle Cloud VM
  ...
  と記録済み（T182の実装メモ参照）。
- 次の一手の候補（優先度はユーザー判断）: ④のDB化（プロセス再起動・Renderのスケール
  アウト時にキャッシュが消える問題への対応）と⑤のバックグラウンド更新（ユーザーリクエストを
  Open-Meteoへの同期フェッチから完全に切り離せる）が、既存実装（①〜③）の上に積む次の
  自然なステップとして有力。あわせて、今回発覚した「全格子点429時でも200 OKで握りつぶす」
  設計自体（`/api/weather`は既に502を返す設計になっているのと非対称）も、根本対策とは別に
  小さく直せる余地がある（フロント側の`windError`表示が自然に機能するようになる）。
