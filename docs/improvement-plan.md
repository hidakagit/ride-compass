...
## 残タスクの優先順位（2026-08-23再整理・第7版）

第6版時点の最優先項目1（T243修正後の本番再実測）はT245として実施済み（王子10km/20kmは
成功、30kmはT243の180秒でも解決しない別要因のDELETE遅延を確認、原因は次回発生時の
ログ待ち）。第7版として更新した。

- **最優先（ユーザー指示: road_graphエンジンの本番化）**:
  1. **T245で仕込んだステージ別ログが実際に本番トラフィックで発生するのを待ち、
     `delete_ms`が支配的な事例が捕捉できたら根本原因を特定・対策する**（現時点では
     再現不能のため待ち状態）。30km級・広域リクエストが本番で継続的に失敗する状態を
     残したままでは、本番化の可否判断ができない。
  2. 上記の解消（またはワークアラウンド確定）を踏まえ、`config.py: routing_engine`の
     既定値切替（現状`"openrouteservice"`）の可否をユーザーと最終確認する。切替判断
     そのものはユーザー裁量（設計原則10に反しない意思決定事項）。
...
T209・T223・T241は調査完了・T242〜T245は実装完了のため本リストから除外した
（T245は「原因はログ待ち」という部分完了だが、実施可能な対応自体は完了済みのため）。
