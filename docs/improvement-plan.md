...
### - [x] T346. 軸スタジオ: ウィザード最終ステップの暗黙送信バグ修正・軸同士の線形結合(nX+mY)テンプレート修正 規模M（2026-08-26完了）
...
### - [x] T353. 内部軸の出力が正規化されないまま他軸の材料として参照される（軸参照と材料正規化方針の不整合） 規模M（完了）
...
### - [x] T354. docs/architecture.mdのAxisComposer.tsx記述をT332後のウィザード構成へ更新 規模S（完了・2026-08-27）
...
### - [x] T356. レビュー履歴ファイル`history/2026-08-26_complexity.md`の不在について記録を整合させる 規模S（完了・2026-08-27）
...
### - [x] T361. axis_definitionsのfresh bootstrapをmigrationシードからDBスナップショット取込みへ転換（migration/API二重管理の恒久解消） 規模M〜L（2026-08-27完了）
...
### - [x] T377. axis_definitionsスナップショットを本番/devの実際の内容へ再生成する 規模S（2026-08-28完了、dev DBのみ）
...
- 実施結果（2026-08-28）: dev機（`backend/.env`が指すローカルPostgreSQL、実データ投入済み）
  に対して`dump_axis_definitions_snapshot.py`を実行。**差分あり**——`bicycle_infra_quality`軸の
  `show_map_icon`が`false`→`true`（T367で本番/devにAPI経由適用済みだった修正が未反映
  だったもの）、`revision`が1→16（T361以降dev DBへ積まれたチューニング回数の裏付け）。
  差分を確認のうえコミット済み。**本番DBに対しては未実施**（このセッションから本番へは
...
### - [x] T365. 目的地（終点）を指定できるルート生成への拡張・ルート結果のクリアボタン 規模M（2026-08-27完了、T365-2でUI再設計）
...
### - [x] T368. 出発地点表示の文字を廃止しピンの色分けだけで伝えるよう簡素化 規模S（2026-08-27完了）
...
### - [x] T369. 推定グループの軸タイルを観測グループと同じ見た目に統一・凡例パネルの画面右端はみ出しを解消 規模S（2026-08-27完了）
...
### - [x] T370. 推定グループを横スクロールへ変更・chipRow/estimatedFlatRowのスクロールバーを非表示化 規模S（2026-08-27完了）
...
### - [x] T372. 出発地点マーカーをドラッグ移動可能にし、現在地アイコンと統一・地図マーカーのtouch-action漏れを修正 規模S（2026-08-27完了）
...
### - [x] T373. chipRow/estimatedFlatRowをJSポインタドラッグによる手動スクロールへ切替（アイコン上からのドラッグでもスクロール可能に） 規模S（2026-08-27完了）
...
### - [x] T376. 地図オーバーレイの`pointer-events: auto`がボタン以外の隙間ごとタッチを奪い、地図パン操作ができなくなる不具合を修正 規模S（2026-08-27完了）
