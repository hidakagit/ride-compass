...
## 残タスクの優先順位（2026-08-23再整理・第9版）

第8版時点の最優先項目1（T246修正効果の実データ検証）は同日中に完了。王子30km
（`way_ids_to_replace=94,470`件、まさに問題を引き起こした規模）を実際に再実行し、
`delete_ms=10.2秒`（修正前は10分超でも未完了）・`candidates=4/8`で正常終了（exit code 0）
することを確認した（T246本文の追記参照）。**T240〜T246を通じてブロッカーとなっていた
road_graphエンジンの本番実行不能状態は解消された**。第9版として更新した。

- **最優先（ユーザー指示: road_graphエンジンの本番化）**:
  1. **`config.py: routing_engine`の既定値切替（現状`"openrouteservice"`）の可否を
     ユーザーと最終確認する**。T236（品質比較、致命的な差異なし）・T241（連結性、
     致命的な問題ではない）・T242〜T246（本番DB起動不能の解消、性能問題の解消）を
     経て、切替の技術的な障害は解消された。切替判断そのものはユーザー裁量
     （設計原則10に反しない意思決定事項）。
  2. T242の残課題（標高バックフィルの定期再実行、新規splitされたEdgeへの反映）を
     自動化するか都度手動実行に留めるかの方針決定。
  3. 本セッションで発見した副次的な観察事項（参考、対応は不要〜任意）:
     - `node_upsert_ms`/`edge_upsert_ms`（バルクUPSERT）が現在の支配的コスト
       （王子30kmの実測で計約195秒）。初回タッチ時のみのコストのため優先度は低いが、
       今後さらに広域のリクエストで問題化する場合は別途調査対象とする。
...
T209・T223・T241は調査完了・T242〜T246は実装完了（改善効果も実データで確認済み）
のため本リストから除外した。
