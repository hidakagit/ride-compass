...
### - [ ] T292. 推定軸に「内部軸→公開軸」の階層構造を導入し、一次材料の宣言的組み立てだけで推定軸を再現・新設できる基盤を作る 規模XL
...
### - [ ] T293. 周回ルートの採用向き（順回り/逆回り）を地図上へ矢印で明示する 規模S〜M — トリガー: 別セッションのT292フロント移行（MapView.tsx等26ファイル）のコミット完了後
...
- **保留する場合の影響**: 現状どおり、逆回りが採用されたかどうかはAPIレスポンスの
  ...
  UX改善機会の見送りに留まる。
- 完了条件: 未実装。着手時、上記5項目を実装・テストし、実機（Playwright）で
  順向き/逆向き双方の矢印表示を確認する。
...
**サマリ（第8回レビュー起票後・19タスク）**: **T279・T289・T274・T281段階1・T282は
完了**（T289は当初T280の実証候補として着手したが、調査の結果MATERIAL_CATALOG対象外の
...
2026-08-25、いずれもフロント作業と競合しないdocs専用タスクとして完了した）。
トリガー未到達16件（T265・T206・T105・T127・T145・T207・T208・T242残課題・
T280・T281段階2〜3・T283〜T288。T145aはT278で解消済みのため除外）／
このほかT273（Phase 4蒸留）・T293（周回ルートの向きを矢印で明示、③案の技術検証済み・
別セッションのT292フロント移行コミット完了待ち）が着手待ち。T275（Tailwind採否）は
2026-08-25に再調査（Tailwind使用箇所は引き続きゼロ、CSS Modulesは30ファイル・
3,302行と実測）したが、ユーザー判断により(c)全面移行案の是非を含め方針決定を保留中。
