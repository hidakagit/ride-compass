...
- **T259との関係（2026-08-24）**: 上記の新宿10km（split済み材料の冷読み出しのみ）で
  ...
  の見直しが必要か、ユーザー判断を仰ぐ。
  - **2026-08-24追記・同日中に切り戻し**: 下記1.のCOPY実装は本番デプロイ直後から
    バックエンドプロセスのクラッシュ→自動復旧の繰り返し（DB非依存の`/api/debug/stats`
    まで巻き添えで`Failed to fetch`になる、ユーザー実機で複数回確認）を引き起こしたため、
    実装当日中に`git revert`で切り戻した。詳細・原因の暫定仮説はT261参照。以下は
    切り戻し前の実装メモ（記録として残す）。
...
- 対応候補（着手時に設計判断）:
  1. **バルクUPSERTの最適化**（2026-08-24実装完了）: `_bulk_upsert`（複数行VALUESの
     ...
     - **切り戻し（2026-08-24、本番障害を受けて実装当日中）**: T261参照。
       `_asyncpg_connection`・`_copy_upsert_road_nodes`・`_copy_upsert_road_edges`・
       関連する`save_graph`の変更・`benchmarks/bench_save_graph_copy.py`はすべて
       `git revert`済み。`_bulk_upsert`（chunk=1000のON CONFLICT、切り戻し後の現行実装）
       のまま。再挑戦する場合はT261の原因調査結果を踏まえた設計にすること。
- 対応候補（着手時に設計判断）:
  1. **バルクUPSERTの最適化**（2026-08-24実装→本番障害により同日切り戻し、T261参照）:
     `_bulk_upsert`のINSERT ... ON CONFLICT一括化の粒度・asyncpg COPYの利用（PBF取込
     バッチが既にCOPY直行の前例を持つ）・「分割結果が前回と変わらないEdgeはUPSERT自体を
     省略する」差分検出等。COPY方式はdev DB・本番実測いずれも劇的な高速化（5.6〜9.6倍）を
     示したが、本番デプロイ後にバックエンドプロセスがクラッシュ→自動復旧を繰り返す障害を
     引き起こしたため一旦切り戻した。再挑戦する場合はT261の原因調査（メモリ使用量の
     疑いが濃厚）を踏まえ、一括COPYではなく境界付きチャンクでのCOPY等、メモリ使用量を
     抑えた設計にすること。
...
## 残タスクの優先順位（2026-08-24再整理・第16版）

第15版以降、T221 Part 3（Stage D）は完了した。T242の残課題（標高バックフィルの定期
再実行）は「当面は手動実行、新規インフラは立てない」という暫定方針で運用するが、
**タスクとしては完了させず、トリガー未到達の継続監視項目として残す**（新規splitの
頻度増加・欠損起因の品質劣化報告のいずれかが起きたら自動化を再検討、T242エントリの
「暫定方針」参照。いずれ自動実行が必要になる可能性があるため、というユーザー判断）。

- **参考記録（対応は不要〜任意、監視のみ）**:
  - T241で見つかった一部方位での「経路が見つからない」事象（8方位中平均1〜2方位）は
    残存するが、実運用への影響は限定的と評価済み（T241本文参照）。

- **トリガー未到達（現時点では着手しない、ただし1.は要ユーザー判断）**:
  1. **T248**（road_graph既定エンジンの性能改善、規模M〜L）: 一般公開の意思決定時に
     必須化、それまでは研究利用での体感遅延報告で着手——という従来のトリガーだったが、
     **T259（2026-08-24）で「未split・20km規模の地点は本番HTTP経由でRenderの約100秒
     プラットフォームタイムアウトに到達し完全失敗する」ことを実地確認済み**。単なる
     体感遅延ではなく既に実際の失敗が起きているため、このトリガー設定（体感遅延の
     "報告"を待つ）が現状に対して適切かどうかはユーザー判断が必要。保留すると、
     未split地点への20km以上のルート生成要求（研究利用中の実利用パターンとして
     十分起こりうる）が今後も原因不明のまま「fail to fetch」で失敗し続ける
     （T105と合わせて2件目の、根本原因判明済みだが未着手のプラットフォーム制約由来の
     失敗）。冷46秒/温6.7秒の実測・対応候補は本文参照。バルクUPSERTの支配的コスト
     （旧・参考記録）とT229（冷パス計測、実測完了により統合クローズ）を本タスクへ
     集約した。
...
**サマリ（10タスク）**: 指示待ち0件（ただしT248のトリガー見直し要否はユーザー判断待ち、
上記1.参照）／トリガー未到達9件（T248・T206・T145a・T105・T127・T145・T207・T208・
T242残課題）。T209・T223・T241は調査完了・T242本体（migration 0013適用・標高
バックフィル初回実行）・T243〜T247・T249〜T254・T256〜T259は実装/調査完了・
T255はDynamicLayerTimeSlider完了（ホイール操作は実機確認済み）・BottomSheetは断念で
完了扱い・T229はT248へ統合クローズ・T221はPart 3（Stage D）まで実装完了（Stage Eは
別タスクとして起票予定）のため、いずれも本リストの指示待ちから外した。安全網の回復3件
（T225・T224・T230）とレビュー指摘の消化5件（T226〜T228・T231・T232）、および
T222（Overpassライブ経路削除）は全て完了済み。T258で追加したデバッグログはT105の
次回再現時の切り分け材料になる想定だが、T105自体はトリガー未到達のまま残る。
T259（20km失敗の根本原因特定）は調査として完了したが、その結果はT248のトリガー再検討
という新たな指示待ち事項を生んだ。
