...
- [ ] [T1021](records/tasks/T1021.md). 補正後の手順で、段階1やり直しの24ファイルを起こし直す 規模L — 残り: 起こし直しで出た保留3件の答えを受けた実装の直し（段階Eと同じ形、1件ずつS） — トリガー: ダッシュボードの T1021-D3-bin-field-guard・T1021-D4-tuning-refresh-after-commit・T1021-D6-prewarm-unreachable-filter のどれかに答えが出たとき
...
- [ ] [T1095](records/tasks/T1095.md). 経路探索エンジンのテストが持つ探索構造の偽物（FakeLazyGraph・FakeCsr等7種）を、本物の型で作る形へ置き換える 規模M
- [ ] [T1096](records/tasks/T1096.md). 廃止したmigrationを現在形で案内している文言・コメントを、今の仕組み（ORMの宣言からcreate_tables()が作る）に合わせる 規模S
