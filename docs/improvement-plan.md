...
- [ ] [T1103](records/tasks/T1103.md). 地図でルートの線を押しても候補を切り替えない（候補の切り替えは一覧のタブだけ） 規模S
- [ ] [T1104](records/tasks/T1104.md). パネルのボタンを、コンパクトな文字入りアイコンのボタンへそろえる（ルート合成の入口を候補の中へ・クリアを全候補を消すと読める形へ・候補の行を所要の長さで崩さない） 規模M
- [ ] [T1105](records/tasks/T1105.md). 不明・データなしを、地図と凡例で不明らしく見せる（区分の分からない道を破線にする等） 規模M
- [ ] [T1106](records/tasks/T1106.md). 難易度で塗るレンズの段の配色を、隣の段が見分けられる形にする（緑→黄→橙→赤、明度も動かす） 規模S
- [ ] [T1107](records/tasks/T1107.md). ルートの風の表示を区間の通過時刻の風にそろえ、区間の詳細に使った風（予報の地点・時刻・風向風速）を出す 規模M
- [ ] [T1108](records/tasks/T1108.md). ルートの風を地点ごとの予報で引く（今は出発地点1か所の予報を全区間に使う） 規模L
