...
- **関連（2026-08-24）**: 「20kmでルート生成に常に失敗する」報告の調査中、本タスクで
  ...
  T258で横断的に修正済み。20kmバグ自体の根本原因（Renderプラットフォームタイムアウトの
  疑い、未確定）はT258でも確定できず本タスクへ引き続き残る。次回再現時はT258で追加した
  `api:route`カテゴリのタイムアウト/通信エラーログが切り分けの材料になる見込み。
...
**サマリ（10タスク）**: 指示待ち0件／
トリガー未到達9件（T248・T206・T145a・T105・T127・T145・T207・T208・T242残課題）。
T209・T223・T241は調査完了・T242本体（migration 0013適用・標高バックフィル初回実行）・
T243〜T247・T249〜T254・T256〜T258は実装/調査完了・T255はDynamicLayerTimeSlider完了
...
本リストにトリガー成立済みの指示待ちタスクは現時点で無い（季節・利用実績・製品判断・
継続監視待ちの9件のみ残存）。T258で追加したデバッグログはT105の次回再現時の切り分け
材料になる想定だが、T105自体はトリガー未到達のまま残る。
