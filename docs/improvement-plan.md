...
### - [ ] T210. 通行止め・道路規制情報の導入可否を調査する 規模S〜M（調査のみ）
...
- **すぐ着手できる（トリガー無し、優先度順）**:
  1. **T174**（夏季WBGT警告、規模S〜M）: 提供期間が4〜10月で、今（8月）はまさに
     提供期間の中盤。T205（警報・注意報バッジ）で共有表示コンポーネント
     （`WarningBadge.tsx`）を作ったばかりのため実装コストが下がっており、
     シーズン終了前に間に合わせる価値が最も高い。
  2. **T176**（河川敷冠水の調査、規模S〜M）: 台風シーズン真っ只中の今が調査の好機
     （既存記載どおり）。T210と表示形式（警告バッジ vs 規制区間ハイライト）を
     共有できる可能性があるため先に済ませる。
  3. **T210**（通行止め・道路規制の調査、規模S〜M）: T176調査後に着手すると
     表示形式の判断を使い回せて効率的（本文の依存記載どおり）。
