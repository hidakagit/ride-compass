...
### - [ ] T299. 「開発者」タブ廃止＋「ルート詳細」タブの設定/結果2分割〔P3〕規模M — トリガー: 現在進行中の別フロント改修の完了後
...
- **トリガー未到達（現時点では着手しない）**:
  ...
  18. **T299**（開発者タブ廃止＋ルート詳細タブの設定/結果2分割、規模M）: 現在進行中の
      別フロント改修の完了後。ユーザー合意済み・着手方針も確定済みだが、並行作業との
      競合を避けるため意図的に保留。

いずれもトリガー未到達の実装を「ついで」にやらない（設計原則10）。

**サマリ（第8回レビュー起票後・19タスク）**: **T279・T289・T274・T281段階1・T282・T294・
T293は完了**（T289は当初T280の実証候補として着手したが、調査の結果MATERIAL_CATALOG対象外の
...
明文化）から方針転換して現状維持とした）。
トリガー未到達17件（T265・T206・T105・T127・T145・T207・T208・T242残課題・
T280・T281段階2〜3・T283〜T288・T299。T145aはT278で解消済みのため除外）／
このほかT273（Phase 4蒸留）が着手待ち。T275（Tailwind採否）は
