...
- **ユーザー提供のRenderログによる追加調査（2026-08-24）**: ユーザーから該当時間帯の
  ...
  - **結論（暫定）**: `build_road_graph`のイベントループ専有は実在する問題だが、
    ...
    有力候補として浮上した**。
- 次の調査候補（未着手、ユーザー判断待ち）:
  1. Renderダッシュボードの当該時間帯（2026-08-23 22:55〜22:57 UTC・23:08〜23:10 UTC
     付近）のログ・メトリクス（CPU/メモリ使用量グラフ・再起動理由）をユーザーに
     確認してもらう（本セッションからは見えない情報）。
  2. **Renderのインスタンスプラン（CPU/メモリの上限）を確認する**。無料/低スペック
     プランの場合、増強（有料プランへの変更）が最も直接的な解決策になりうる
     （コード修正では解決しない可能性がある）。
  3. プラン増強が難しい場合、根本対応は冷パスの計算コストそのものを削減する方向
     （T248候補2「冷パスの体験設計」＝リクエスト同期からの切り離し・バックグラウンド
     ジョブ化等）になる。規模はL以上になりうる。
- 完了条件: 未達（原因未確定、コード修正2件では解消せず）。Renderダッシュボード側の
  CPU/メモリ情報が得られ次第、方針を確定する。
