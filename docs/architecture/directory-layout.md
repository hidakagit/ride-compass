...
この3つの向きは`frontend/eslint.config.mjs`の`import/no-restricted-paths`が持ち、破るとCIのeslintで落ちる
（機能の境界は`features/`の下のディレクトリから導くので、機能を足しても設定は書き足さない）。テストも同じ向きに従い、
機能と`app/`の両方を読むテストは`app/`に置く。
