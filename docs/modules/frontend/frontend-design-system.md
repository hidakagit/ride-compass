...
### 地図の上に重なる操作部品は、ピンチをページへ渡さない

viewportはページの拡大を許している（アクセシビリティのため`maximum-scale`を置かない）。
そのため`touch-action`が既定（auto）の部品に2本指を置くと、地図ではなくページ全体が拡大する。
これを部品ごとの指定ではなく、`globals.css`の1つの規則で「地図ペイン（`.app-map-pane`）の直下に
重なる要素（地図を包むMapViewの要素を除く）」と「MapLibreが地図の中へ置く操作部品
（`.maplibregl-ctrl`）・ポップアップ」へ`touch-action: none`を掛けて持つ。ピンチの許可は祖先との
積で決まり、Chromiumではスクロール容器を越えても受け継がれる（スクロール容器が戻すのはパンだけ）
ため、直下の要素へ掛ければ中の部品すべてに効く。地図に重なる新しい部品は、地図ペインの直下に
置けばこの規則に入る。部品が個別に持つ`touch-action`（チップの`none`・パネルの`pan-y`等）は、
ドラッグを地図とスクロールのどちらへ渡すかを決めるためのもので、この規則とは役割が別である。

効いていることは`e2e/map-runtime.spec.ts`の「タッチ端末」が、地図の範囲にある部品全部へ
2本指のピンチを送って確かめる。Radixのポータルで`body`直下へ出るポップオーバーは地図ペインの
外なので、この規則の外にある。
