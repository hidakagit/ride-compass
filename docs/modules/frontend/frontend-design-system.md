...
`e2e/map-runtime.spec.ts`の「タッチ端末」が、地図の範囲にある部品へ2本指のピンチを送ってページが
拡大しないことと、地図の上のピンチが地図を拡大することを確かめる（ヘッドレスのChromium）。下部シート・
ポップオーバー・ヘッダーの中から始めたピンチは、まだE2Eで確かめていない。iOS Safariの挙動は公式の
文書とWebKitのソースを根拠にしている。
