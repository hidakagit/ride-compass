...
| ファイル | 責務 |
|---|---|
| `features/map/scene/groups/roadLines.ts` | 道路の線（路面の種類・道路の種類・トンネル・一方通行）の分類・配色と、詳細を見ている1本の強調。**線レイヤーで分類を運ぶのは色だけで、太さは情報を持たず、線種が運ぶのは値が無いこと（タグが無い道＝「不明」の破線）だけ**——1本の線へ複数の分類を載せると、色の意味が他方のON/OFFで入れ替わる。同時表示は横へ平行に割り付けて分ける |
...
| `features/map/MapView/roadFacts.ts` | クリックした道の「事実」（道路名・路面・路面の状態・トンネル・橋・一方通行）をタイルのプロパティから組み立てる純関数。項目名と値の呼び名、タイルのどの属性を読むか（`tile_property`）は材料カタログ（生成物`material-catalog.json`）から引く（路面の舗装の有無は、カタログに値の呼び名が無いので画面が持つ）。該当しない項目は行ごと出さない（「なし」が並ぶと該当する項目が埋もれる） |
...
源泉が配る値（`mapDisplay.road.lineWidthPx`）を使い、線種は値が無い道（「不明」）の破線にだけ使う。両方ONのときは下記の並列トラックが横へ分ける。
...
**太さ・線種が運んでよいのは「操作の状態」と「値が無いこと」だけ**——選んでいるかどうかは、色では表せない
...
凡例の「横断歩道」に残るのは信号を伴わない横断歩道だけである。まとめる単位は凡例の行で、
凡例で同じ行に入る近くの点（車道用と歩道・自転車道用の踏切）は1点で届く。補給休憩POIはまとめずに
1点ずつ届く。
