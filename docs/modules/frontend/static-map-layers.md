...
タイルには材料の値が焼き込まれている（例: `surface_class`［路面の区分］・`surface_good`［舗装良否、
線の分類としては未使用］・`highway`［道路種別］）。線で描くのは、分類と色を宣言した属性だけ
（例: 路面の種類・農道・林道の等級・道路の種類）。

**線で描く分類を足すときは、「他の分類と独立して決まる事実」であること。** 粒度違いの
再掲（`surface_good`は路面の区分を2値へ粗く束ねただけ）は足さない——「路面の種類=砂利」
...
無いのは見落としではなく、束ねないと決めた結果である。**路面の束ね方はbackendの区分の宣言
（材料「路面の区分」）そのもので、地図の行はその値を1行ずつ塗る。
