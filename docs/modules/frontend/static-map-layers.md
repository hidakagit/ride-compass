...
**その分類が評価へ寄与するかは、色を決める前にbackendで確かめる。** 評価軸の計算式が実際に
その分類を材料として使っているもの（例: 事故の当事者区分は事故密度の材料で、自転車関連だけが
寄与する）は2次と同じ配色へ揃え、どの軸の材料でもないもの（例: 一方通行）は中立色にする。
UIの見た目だけで決めない。

**寄与していても、順序が運用で変わるものには評価配色を使わない。** 停止要因POIの種別は
評価へ効く（`domain/traffic.py`の`POI_COUNT_KINDS`が停止密度の材料になる）が、種別ごとの
重みは軸定義（`axis_definitions`、軸スタジオで編集する）が持ち、運用で入れ替えられる。
フロントへ順序＝評価配色を焼き込むと、重みを入れ替えた瞬間に地図の色の読み方が軸の定義と
食い違う。
