...
台の中の見本は地図と同じ形にする（道の線の行は線、点の行は点。凡例の行の`line`）。値が無い行
（凡例の行の`isFallback`。「不明」「データなし」）は、地図の破線と同じく途切れた見本にする
（`legendFilter.ts: legendSwatchBackground`。チップの▶・レンズの凡例とピルが同じ関数を通る）。台を色の周りの
細い輪にすると、輪が色の部分より目立つ。
