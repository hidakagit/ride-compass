...
| ファイル | 責務 |
|---|---|
| `features/map/scene/groups/roadLines.ts` | 道路の線（路面の種類・道路の種類・トンネル・一方通行）の分類・配色と、詳細を見ている1本の強調。**線レイヤーで分類を運ぶのは色で、太さは順序のある分類（道路の種類）だけが色と同じ順序を重ねて示し、線種が運ぶのは値が無いこと（タグが無い道＝「不明」の破線）だけ**——1本の線へ複数の分類を載せると、色の意味が他方のON/OFFで入れ替わる。同時表示は横へ平行に割り付けて分ける |
...
**どちらの色の意味も、もう一方のON/OFFでは変わらない**。太さは源泉が配る共通の値（`mapDisplay.road.lineWidthPx`）で、
順序のある分類の行だけが源泉の配る行ごとの太さ（生成物`primaryAttributes.ts`の`line_width_px`）を持つ。線種は値が無い道
（「不明」）の破線にだけ使う。両方ONのときは下記の並列トラックが横へ分ける。

分類に当てはまらない道は2種類を分ける。**タグが無い道は「不明」**（値が無い。薄い灰の破線、刻みは
...
値の無い道が現れないので、「不明」の行も破線も持たず、分類の外の行を「該当なし」と呼ぶ。
...
道路と平行な複数トラックへ横並びに分離する——**出ている線から割り付け、帯の中央を道の位置に置く**ので、
どれかをOFFにすると残りが自動で中央（実際の道路の位置）へ寄り直す。隣どうしは互いの太さの半分ずつ
離したうえで、源泉の重ね幅（`mapDisplay.road.trackOverlapPx`）だけ重ねる（離すと1本の道が複数に見える）。

**太さが道ごとに違うので、横位置も道ごとの式になる。** 道路の種類の線は幹線ほど太いため、間隔を1つの
定数にすると、太い線の隣では相手を覆い、細い線の隣では隙間が空く。二次軸の下敷き（下記）の幅も、
全部の線が最も太いときの帯の幅から導く。
...
覆いを重ねる。外すのは基礎地図が描けた時点で、MapLibreの`load`で取る——`load`は載っている
すべてのタイルが揃った最初の描画で来るが、アプリのソースは`load`の後に足す（`runWhenStyleReady`）
ので、その時点で描けているのは基礎地図だけ。`idle`は表示中のすべての取得が落ち着くまで来ないので、
使うと遅い外部データが1つあるだけで描けた地図を覆ったままになる。基礎地図のタイルが止まって
`load`が来ないときの保険に、`INITIAL_TILES_OVERLAY_MAX_MS`（6秒）でも外す。
...
出す（凡例の行の`lineWidthPx`）ので、太さで順序を示す行（道路の種類）は見本でも太さを見比べられる。点の見本は
...
値が無い行（凡例の行の`isFallback`。「不明」「データなし」）は、地図の破線と同じく途切れた見本にする
（`legendFilter.ts: legendSwatchBackground`。チップの▶・レンズの凡例とピルが同じ関数を通る）。
...
未分類）は消さずに不透明度を下げる。1次のレイヤーを複数同時にONにしても、分類情報を
持つ区間だけが浮かび上がる。**薄い側も地図の上で見える濃さに留める**——灰の線が基礎地図の地色に対して
コントラスト比1.5を割ると、道があるのに線が無いように見え、「不明」とも読めない（値はbackendの
`map_display.py`、測るのは`tests/test_primary_attribute_display.py`）。評価軸の下敷きの薄さはこれとは別の値で、
材料の線より手前に重なるため、材料の線の色を隠さない薄さにする。**「対象外」へ評価配色の緑は割り当てない**——対象外は「安全と
確認済み」ではなく「材料が無い」であり、緑が持つ「良い」という含意とは別物のため、
中立グレーで薄く出す。

**分類を運ぶのは色で、線種は分類を持たない。** 同じ道へ複数の線が重なるときは、
色を掛け合わせず横へ平行に割り付ける（1本の線へ2つの意味を載せると、色の意味がもう一方の
ON/OFFで入れ替わる）。

**太さが分類について運んでよいのは、順序のある分類の順序だけ**で、色が示すのと同じ順序（幹線ほど太い）を
重ねて示す。別の分類を太さへ載せるのではないので、上の「1本の線へ2つの意味」には当たらない——色の見分けが
つきにくい人・小さな画面でも、太さで順序が読める。太さはbackendが色と同じ並びの位置から導き
（`domain/display_palette.py`）、細い端は他の線と同じ太さにする（それより細いと見えにくく、押しても拾いにくい
——道を押したときは押した1点に重なる線だけを拾う）。順序を持たない分類には付けない（意味の無い差を読ませる）。
凡例の線の見本も、地図と同じ太さで出す。

**それ以外で太さ・線種が運んでよいのは「操作の状態」と「値が無いこと」だけ**——選んでいるかどうかは、色では表せない
（色は分類に使い切っている）。乗り換え帯が未選択で破線・選択で実線かつ太いのはこれに当たる
（[ルート設定と結果](route-settings-and-results.md)）。値が無いこと（道の「不明」、評価軸・ルートの
