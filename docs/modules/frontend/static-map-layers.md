...
| ファイル | 責務 |
|---|---|
| `features/map/scene/groups/roadLines.ts` | 道路の線（路面の種類・道路の種類・トンネル・一方通行）の分類・配色と、詳細を見ている1本の強調。**線レイヤーで分類を運ぶのは色だけで、太さは情報を持たず、線種が運ぶのは値が無いこと（タグが無い道＝「データなし」の破線）だけ**——1本の線へ複数の分類を載せると、色の意味が他方のON/OFFで入れ替わる。同時表示は横へ平行に割り付けて分ける |
...
源泉が配る値（`mapDisplay.road.lineWidthPx`）を使い、線種は値が無い道（「データなし」）の破線にだけ使う。両方ONのときは下記の並列トラックが横へ分ける。
...
値が無い行（凡例の行の`isFallback`。どの凡例でも`mapColorLegend.ts: NO_DATA_LEGEND_BAND`の「データなし」）は、地図の破線と同じく途切れた見本にする
...
コントラスト比1.5を割ると、道があるのに線が無いように見え、「データなし」とも読めない（値はbackendの
...
（[ルート設定と結果](route-settings-and-results.md)）。値が無いこと（道・評価軸・ルートの
