...
| `components/RouteSplicePanel/RouteSplicePanel.tsx` | 区間の乗り換えの操作面（「ルート結果」が編集モードのときの中身。編集の元・区間ごとの選択［元のまま／候補横断の代替］・差分を見る・新しいルートを作る・やめる）。**区間は元ルートの何km地点かで示し、選べる道はその区間が何km長く（短く）なるかで示す**——候補に名前が無いため、番号と全長では区別がつかない。**距離差の基準はその代替が差し替える範囲**で、行の見出しが示すグループの範囲ではない（グループは範囲が重なる代替をまとめた器で、覆う範囲は代替ごとに違う。グループ全体と比べると、短い範囲を差し替える代替ほど実際より大きく減って見える）。代替ごとの範囲は(i)ではなくチップの`title`・`aria-label`に出る。表示専用で、区間の算出も経路の組み立ても`lib/routeSplice.ts`が行う。**区間は元と候補が同じ地点を通る所で割る**ため、候補が全域で違っていても区間ごとに別の候補を選べる（＝複数候補の乗り継ぎ）。1つの区間の中でも、**候補どうしが触れる地点で道を継いだ代替**が並ぶ（`lib/routeSplice.ts: chainedAlternatives`。乗り換え回数と件数に上限があり、継いだ代替は地図の帯を持たない——1本の候補の座標では描けないため）。選んだ区間の数は見出し行に出し、区間が上限を超えたら選んだ区間＋上から数区間だけ残して畳む。`edge_ids`が空の候補では「差が無い」と「そもそも出せない」を区別して伝える。使い方（帯の見方・選び方）は画面へ書かず見出し脇の(i)の奥に置く。**操作（差分を見る・新しいルートを作る）は文言のボタンではなく見出し行の右端のアイコン**で、「ルート結果」ヘッダーの操作アイコンと同じ枠・大きさに揃える。元ルートと「元との差」は同じ1行に置き、1区間=1行（選べる道が多い区間は行の中で横へ送る）——下部シートの高さが限られるため、操作と補足で縦を使わない |
...
- **評価に使っていない軸**: 重み（生成時点の`route_preference`）が0の軸のチップは**出さない**
  ...
  ルート設定パネルの「重み配分」と共有する（`ui/axisLegend.module.css`）——同じ軸が画面に
...
同じ表現）をそのまま流用した積み上げ1本バーと、その下の凡例（軸アイコン＋数値）を
...
すべてを渡せば、寄与が出ない軸も薄いチップとして残る。

チップを詳細の入口にするかは`renderDetail`（軸→詳細の中身）が決める。prop自体を省略すれば
どのチップも押せない静的な凡例で、**軸ごとにnullを返せばその軸だけ押せない**（結果パネルの
「評価に使っていない軸」がこれにあたる）。**詳細の中身はこのコンポーネントが組み立てない**
...
backendが候補件数を常に1件へ固定し無視するため操作しても無駄だが、消えると壊れて見える。経由地・目的地のいずれも未指定のまま
