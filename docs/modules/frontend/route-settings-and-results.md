...
属する条件に見えるうえ1行ぶん縦を余計に使う）。経由地が無い場合のみ表示する（経由地を
伴う目的地ルートはbackendが候補件数を常に1件へ固定し無視するため）。経由地・目的地のいずれも未指定のまま
...
- 地図のピンは3つとも同じ丸いバッジ（`MapView.tsx: createPointMarkerElement`、出発地だけは
  現在地アイコン入りの白バッジ）で、**どれもつかんで動かせる**。行頭の印は同じ色・同じ字で、
  行とピンを結ぶ。動かした直後のclickは読み飛ばす（`bindDragAwareClick`）——同じ操作の
