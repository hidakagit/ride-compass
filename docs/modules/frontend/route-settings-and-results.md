...
| `components/RouteSplicePanel/RouteSplicePanel.tsx` | 区間の乗り換えの操作面（「ルート結果」が編集モードのときの中身。編集の元・比較相手の選択・区間のトグル・候補への追加・やめる）。表示専用で、区間の算出も経路の組み立ても`lib/routeSplice.ts`が行う。`edge_ids`が空の候補では「差が無い」と「そもそも出せない」を区別して伝える。使い方（帯の見方・選び方）は画面へ書かず見出し脇の(i)の奥に置く |
