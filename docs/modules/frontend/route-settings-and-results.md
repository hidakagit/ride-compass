...
useAxisCatalog() ──→ catalog.axes（公開軸一覧、is_published=Trueのみ）
        ...
        ┌─────────────────┼───────────────────────┐
        ▼                                          ▼
  重み配分バー（帯グラフ、境界              軸ごとの行（1行、常時表示）:
  ドラッグ/矢印キーで隣接2軸の重みを        チェックボックス + ラベル +
  移し替え）→ その下に軸ごとの行            現在値ボタン（タップでポップオーバー：
                                             スライダー+向きコンパス）+「色分け」トグル
                                             ↓
...
- 重み配分バー（帯グラフ、`stackBarOuter`/`stackBarHandle`）は表示専用ではなく、
  ...
  本体）はスクロールジェスチャーを妨げない。0.01刻みでの単独調整・0への変更（実質
  チェックOFF相当）は各軸行の現在値ボタンから開くポップオーバー内スライダーで行う。
- 各軸の行はチェックボックス・ラベル・現在値ボタン・「色分け」トグルだけの1行。
  現在値ボタンをタップすると、Radix Popover（`Popover.Portal`でdocument.body直下へ
  描画）でスライダーが浮遊表示される。風・勾配の「色分け」がONの間は、同じ
  ポップオーバー内にスライダーと並んで向きコンパス（`WindBearingSlider`、
  `renderBearingControl`）も表示される。
