...
- **寄与が出ていない軸**: 寄与度バーに出ない軸（`axis_contributions`が無い・0）は、バーの
  下の折り返しチップ行へ置く。行き先は2つに分かれる。重み（生成時点の`route_preference`）が
  0の軸は「未使用の軸 N本」という開閉ボタン1つへ畳む（既定は畳んだ状態、本数は残るため
  「消さずに薄くする」（[設計原則](../../design-principles.md)「UI仕様」）は畳んだ後も
  成り立つ）。重みは入っているのに値が来ない軸は畳まず「データなし」バッジ付きのチップで
  残す——**畳む条件は重みだけで決める**（値が無いのはユーザーの選択ではないため、読みたい軸が
  黙って隠れない）。どちらのチップも押せば同じ詳細が開く。
...
凡例チップを軸ごとの詳細の入口にするかは`renderDetail`（軸→詳細の中身）の有無で決まる。
渡さない間チップは押せない静的な凡例のままで、**中身はこのコンポーネントが組み立てない**
