...
- **軸別難易度の一覧**: 公開軸すべて（軸カタログ順）を「色ドット＋ラベル＋(i)説明＋難易度
  ...
  **(i)・難易度・生値は軸をまたいで縦に揃える**——行ごとにflexで並べると生値の有無と
  文字数で右側の列位置が行ごとにずれ、読み比べられない。一覧側を5列のグリッドにし、
  各行は`display: contents`で自分の箱を持たない（行ごとにグリッドを作ると列幅も行ごとに
  決まってしまう）。生値の列は値の無い軸でも空のまま置き、`data-unused`の薄さは行では
  なくセルへ掛ける（`display: contents`の要素にはopacityが効かない）。この候補を評価した重み
  （生成時点の`route_preference`）が0の軸は「未使用」バッジ付きで薄く残し、値が無い軸は
  「データなし」を示す。選択操作は持たない（地図の色分けは`LensControl`）。
- **得点の隣の生値**: 難易度の右に、折れ点を通す前の生値を添える
  （`RouteCandidate.axis_raw_values` × `AxisCatalogEntry.raw_value_unit`、
  `axisRawValue.ts: formatAxisRawValue`）。単位が「◯◯/km」なら**候補の走行距離を掛けた
  実数だけ**を出す（例:「約26回」）——密度そのものは隣の得点を決めている当の値で、
  並べても繰り返しになるうえ列幅を倍に食う。距離を掛けられない単位（%等）は生値を
  単位付きでそのまま出す。得点0-100は目盛りの引き方に依存する相対評価
