...
| `components/RouteSplicePanel/RouteSplicePanel.tsx` | 区間の乗り換えの操作面（「ルート結果」が編集モードのときの中身。編集の元・区間ごとの選択［元のまま／候補横断の代替］・差分を見る・新しいルートを作る・やめる）。**区間は元ルートの何km地点かで示し、選べる道はその区間が何km長く（短く）なるかで示す**——候補に名前が無いため、番号と全長では区別がつかない。表示専用で、区間の算出も経路の組み立ても`lib/routeSplice.ts`が行う。`edge_ids`が空の候補では「差が無い」と「そもそも出せない」を区別して伝える。使い方（帯の見方・選び方）は画面へ書かず見出し脇の(i)の奥に置く。**操作（差分を見る・新しいルートを作る）は文言のボタンではなく見出し行の右端のアイコン**で、「ルート結果」ヘッダーの操作アイコンと同じ枠・大きさに揃える。元ルートと「元との差」は同じ1行に置き、1区間=1行（選べる道が多い区間は行の中で横へ送る）——下部シートの高さが限られるため、操作と補足で縦を使わない |
