...
| `components/RouteSplicePanel/RouteSplicePanel.tsx` | 区間の乗り換えの操作面（比較相手の選択・区間のトグル・候補への追加）。表示専用で、区間の算出も経路の組み立ても`lib/routeSplice.ts`が行う。`edge_ids`が空の候補では「差が無い」と「そもそも出せない」を区別して伝える |
...
- **評価に使っていない軸**: チップは公開軸すべてを**ルート設定パネルの「重み配分」と同じ形**
  （共有CSSは`ui/axisLegend.module.css`）で並べ、重み（生成時点の
  `route_preference`）が0の軸は薄いチップのまま**押せなくする**——押せるかどうかが、その軸を
  評価に使ったかどうかの区別になる（[設計原則](../../design-principles.md)「消さずに薄く
  する」）。同じ軸が設定画面と結果画面で違う形に見えると、設定した軸と結果に出ている軸が
  同じものだと読み取れない。重みは入っているのに値が来ない軸のチップは押せるままで、
  詳細が「データなし」を示す。
...
目的地チップは「指定する／置き直す」だけを担い、押すたびに武装をトグルする。設定済みから
武装しても目的地は残したままで、次の地図タップが置き換えになる——生成後に目的地を変えたい
とき、解除してから指定し直す2段階を踏ませないため。解除は設定済みのときだけ出る別ボタン
（✕、経由地チップと同じ形）。状態（未設定/タップ待ち/設定済み）は`title`・`aria-label`
だけでなくチップ内の文言でも示す——モバイルにツールチップは無く、アイコンだけでは
「次に何をすればよいか」が画面から読めない。
