...
（[T518](../../tasks/T518.md)）。呼び出し側（page.tsx）は`axes`をルート設定の重み>0の
軸のみへ絞り込んで渡す。
...
- `ComparisonPanel.tsx`: 研究モードの実験スロット比較表。表示順は
  ...
  （「今」の設定）は使わない。複数の実験スロットを横断比較する画面のため、あるスロットを
  ある軸の重み>0で生成した後にその軸の重みを0へ変更しても、比較したい軸の行が
  表から消えないようにする（改善計画T524、T518コードレビュー指摘の是正。
  「今」の設定を基準にすると、重みを変えて何が変わったか比較するという比較タブ本来の
  目的と逆行していた）。
