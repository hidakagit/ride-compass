...
3. **常時見せるのは状態の合図まで、文は1タップ奥。** 地図に重なるUIは視界を削らないため、
   常時出すのは状態ドット（`components/ui/statusDot.module.css`）のような小さな合図に留め、
...
警報・注意報系は[API設計](../../architecture/api-design.md)のとおりbackendがfail-openで空を
返すため、フロントが受け取る失敗は通信エラー・429だけで、これも「警告なし」として握る
（`hooks/useWeatherConditions.ts`）。現状はこの失敗を画面に出さない——バッジが0件の
ときは画面に置き場自体が無く、見せるには常時の表示を1つ足すことになるため、規約3だけでは
決まらない。
