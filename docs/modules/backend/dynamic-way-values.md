...
- **指定方位に対して直角に近い道路は、勾配の値を持たせずに結果から落とす**
  （`domain/gradient.py: shows_gradient`、地図では「データなし」）。`effective_gradient`は
  直角で0へ近づくが、0は「平坦」を意味する値として凡例が既に使っている——0%として配ると、
  実際には急な坂の道が平坦な道と同じ色で塗られ、読み手には区別できない。その向きについて
  言えるのは「勾配を示せない」であって「平坦だ」ではない。落とす幅
  （`LENS_PERPENDICULAR_BAND_DEG`）は実地を見て決め直す値。
