...
寄せてよい範囲には2つの限度がある。**どちらも無いと、利用者が指した地点とは別の場所を
指定したことになる**（呼び出し側は返ったNodeを「指した地点」として扱う）。
...
- **`RouteCandidate.node_ids`は経路が通るNodeを`edge_ids`より1件多く持つ**
  （`node_ids[i]`が`edge_ids[i]`の始点、末尾が終点）。フロントは「候補どうしが同じ地点を
  通るか」をこれで判定する——座標の完全一致でも実質は一致するが、乗り換えを鎖で伸ばすほど
  その判定が結果を左右するため、グラフが持つ同一性をそのまま渡す。
