...
ステータス: **実装完了（2026-08-25起票・同日Stage A/B実装＋実機確認まで完了）**。
Gap1・Gap2に加え、実装中の追加要望に応じてmaterials統一（`axisMaterials`/
`axisMaterialLayerIds`の静的json依存解消）・`STATIC_FILTER_AXES`の実行時フェッチ化・
既存軸限定の特別扱いコードの洗い出しと除去も同一の流れで実施した。詳細は
docs/improvement-plan.md T308本文を参照。
