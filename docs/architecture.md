  ...
  frontend/
    ...
    src/
      ...
      components/
        ...
        WarningBadge/WarningBadge.tsx ✅ 改善計画T205: 警報・注意報バッジ（地図レイヤーではなくバッジで表現する警告表示の共通コンポーネント）。JMA固有の型に依存しない汎用item形のため、T174（WBGT警告、未着手）も同じコンポーネントを再利用する想定
...
ないトレードオフをT174（WBGT警告、未着手）と共有する）。
