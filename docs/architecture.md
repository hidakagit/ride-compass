...
### 風評価・総合スコアリング・難易度可視化の初期設計（Step7〜9）
...
- **レイアウト**（[frontend/src/app/page.tsx](../frontend/src/app/page.tsx)）: `display:flex; height:100vh`のルート要素の下に、折りたたみ可能な`<aside>`（左サイドバー。現在の中身は「ルート設定」「ルート結果」「ルート編集」の開閉区分で、構成は[modules/frontend/page-composition.md](modules/frontend/page-composition.md)が持つ）と`flex:1`の地図ペイン（`MapView`＋地図上の`MapOverlayControls`）を並べる。位置情報（現在地取得・手動入力）の状態は`MapView`から`page.tsx`（`Home`）に引き上げ、`MapView`は`location`等をpropsで受け取る「地図描画に専念する」薄いコンポーネントにした。
...
`route_preference.yaml`（撤去済み）の手書きミラーを撤廃、軸スタジオが唯一の情報源になった）：
...
（改善計画T316でこの既定値の情報源を一本化、旧`route_preference.yaml`の手書きミラーは撤廃済み）。
...
後方互換）。`components/Map/secondaryAxes.ts`の旧`SECONDARY_AXIS_LAYER_IDS`は、
...
- 上記に伴い、既存軸だけを特別扱いしていた静的定義（旧`AXES_WITH_INPUTS`・
  旧`CatalogAxisInputs`等）は死んだコードとして削除した。動的ramp（時刻依存の合成表示、
...
（フロント`SECONDARY_AXIS_ICONS`/旧`RAMP_AXIS_PANEL_HINTS`/`SECONDARY_AXIS_CHIP_LABELS`/
旧`SECONDARY_AXIS_PROXY_HINTS`、backend`axis_display.py`の`STOP_DENSITY_DISPLAY`等）として
...
1. `derive_ramp_inputs()`が成功 → 自動導出した`tile_inputs`を使う。
   `display_thresholds_override`が設定されていれば色分けのしきい値だけそちらで置き換える
2. 失敗 → `kind="none"`（地図表示を持たない）

**`derive_ramp_inputs()`が扱えるもの**（`domain/axis_display.py`）:

- **軸参照の再帰解決**: `MaterialTerm.material`が材料idではなく別の軸idを指す階層構造でも
  解決する。ただし安全に変換できるのは、参照先が`CategoricalShape`のとき（値をそのまま
  返す）と、単一term・weight=1.0・preprocess="identity"の`BreakpointLinearShape`のとき
  （自己変換材料としてそのまま表現できる）だけで、それ以外は安全側でNoneを返す。
  循環参照は`visited`集合で防ぐ。
- **実行時スケールが要る材料**: タイルの生値から材料スケールへの変換係数が静的に決まらない
  材料は`TileInputSpec.needs_runtime_scale`で印を付けるにとどめる。係数は
  `GET /api/axis-catalog`が`material_runtime_scales`（tile property名→係数）として別途返し、
  フロントの`axisLayers.ts: rampAxesFromCatalogAxes()`が構築時に一度だけ`weight`へ掛ける
  （未解決なら`weight=0`の安全側へ倒す）。
- **真偽値だがタイル側は文字列の材料**: `tile_property_categorical_true_values`で「trueに
  該当する値」を宣言する（`is_designated`のように、タイルでは複数値の文字列プロパティとして
  表現される材料向け）。

**色分けの刻みは別フィールドで上書きできる**。`derive_ramp_inputs()`のしきい値は
`AxisDefinition.shape.breakpoints`のX軸値をそのまま流用するため、複数材料の組み合わせや
単純な線形正規化では粗い段階しか作れない。これは自動導出の能力ではなく刻み方の好みの問題
のため、`display_thresholds_override`へ分離してある（管理APIが昇順・非空を検証し、
軸スタジオGUIが数値配列として編集する）。

`AxisDefinitionResponse`は`axis_display_for()`の計算結果を`display`として返す。**下書き軸は
`GET /api/axis-catalog`に現れない**ため、軸スタジオが「この軸は地図表示の経路を持たない
（`kind="none"`）」と注記できるのはこの管理APIの応答だけである。
...
adjustment"]`——改善計画T292で専用Pythonレシピの`motor_vehicle_no_override`［撤去済み］から移行済み）
...
- **車ストレス**（改善計画T150で「交通ストレス」から改称）: 改善計画T292で専用Pythonレシピを
  ...
  旧`domain/axis_definitions.py: car_stress_display_level`）は末端消費者ゼロと確認の上
...
（RoadGraphEngineが使う）。サンプル点列を渡すKNN空間マッチ版（旧`get_nearest_stop_poi_counts`
...
収録年数で「件/(km・年)」へ正規化する旧`distance_weighted_accident_density`は、
...
### 外部ソースの生データを1つの形で持つ（`source_features`・`source_runs`）

取込元ごとに表を分けると、ステージング・MERGE・run記録・絞り込みの適用が取込元の数だけ
増える。`source_features`は外部ソースの生データを**ソースによらない1つの形**で持ち、
取込の経路を1本にする。ソースごとに違うのは外部の形を読むアダプタだけになる。

点・線・面を同じ骨格へ載せるため、**ラスタはタイル1枚を1行**として扱う。列は
「識別子（`source`＋`natural_key`）／範囲（`geom`）／属性（`attrs`）／実体（`payload`）」で、
`attrs`は外部が持っていた属性をそのまま入れる袋、`payload`は属性として読めない配列実体
（ラスタの画素・OSMの参照ノードid列・ベクタタイルの本体）を置く。

`source`でリスト・パーティションする。あるソースを丸ごと差し替えても他のソースの行を
巻き込まない。子パーティションは取込の側が作る——どのソースが在るかはデータで決まる。

`source_runs`は取込1回ぶんを1行で持つ。`profile`にそのとき適用した絞り込みの宣言を丸ごと
入れるため、「範囲を変えて取り直した」のか「同じ範囲を取り直した」のかを後から区別できる。
派生データはこの`run_id`を指し、自分が指すrunが最新でないことで古いと分かる。

### 生データから導いたものは粒度ごとに1表（`road_edges`・`edge_materials`・`way_materials`・`node_materials`）

派生の表をバッチごとに作ると、材料を1つ足すたびに表もバッチも増え、読む側は表の数だけ
LEFT JOINを組み立てることになる。**粒度で分ける**——点に付く値は`node_materials`、
道1本に付く値は`way_materials`、交差点で切った区間に付く値は`edge_materials`。バッチが
増えても表は増えず、増えるのは列だけになる。

`road_edges`は区間の形（線・長さ・方位）を持ち、`edge_materials`がその区間に付く値を
持つ。**区間は向きで分けない**——向きが持つ情報はいずれも導ける（方位は逆順の始点・終点
から求め直し、勾配は符号を反転し、通行の可否は道の属性から決まる）。有向グラフは探索が
メモリ上で組む実行時の構成物である。

値の列は**未計算をNULLで表す**。行は`road_edges`と同時に作り、値を出すバッチがそれぞれ
自分の列を埋める。どの列がまだ埋まっていないかは「その列がNULLの行数」で一様に数えられる。

面（ラスタ）の派生は持たない。面の生データを読む出口は「そのまま見せる」か「線へ落とす」
のどちらかで、面のままの中間結果を要る相手がいない。

道1本の値が区間の値と重なるのは、低ズームのタイルが道1本を1フィーチャーとして塗るため。
**重なる値は区間から集約して作る**ので、地図が塗る値と評価が読む値が食い違わない。
