...
- **レイヤー順序**: `ensureGsiReliefLayer`（`MapView.tsx`）は地図初期化直後に一度だけソース/レイヤーを追加し、以降はvisibilityの切替のみで表示・非表示を行う。面で塗るレイヤーは基礎地図が最後に面を描いた位置の直後へ差し込まれる（`mapStyleOps.ts: areaLayerAnchor`）ため、基礎地図の道路線・ラベルも、後から追加される路面・ルート系のレイヤーも、必ずこのラスタの上に重なる。不透明度は面で塗るレイヤー共通の値（`AREA_LAYER_OPACITY`）で、こちらは面そのものが背景と区別できる濃さだけを決める。
...
- フロント（`AxisComposer.tsx`）の旧`displayOverride`素通し保持コードも削除した
