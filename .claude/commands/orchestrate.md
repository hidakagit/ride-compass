...
- 回の途中で司令塔のセッションを替えて起動したときは、`board run start`をしない（回を置き換える）。規約の
  「セッションの交代」節のとおり、引き継ぎ（`board run list`）と状態の表・ダッシュボード・司令塔の作業物
  （`python scripts/orchestrate.py paths`の`coordinator/`）を読んでから続ける。会話の要約の後も同じものを読み直す。
