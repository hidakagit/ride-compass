...
└── history/         # レビュー結果の蓄積（YYYY-MM-DD_<type>.md）、規模ウォッチの前回値（size_watch.json）
```
...
`cmd_docs`が実行時に行う配線の自己申告）。**揃っているのは「どの検知器を数えるか」までで、
「どこを走査するか」は経路ごとに違う**——pre-commitはステージ済みの差分、CIはその範囲の
差分、範囲指定なしの全件実行はリポジトリ全体を見る。片側にしか無い検知器は「手元では止まるのに
CIでは素通り」になる。

検知器が**実際に鳴るか**は`review_checks.py mutate`が確かめる（わざと違反を1件入れて
落ちるかを、pre-commit経路・CI経路の両方で試す。使い捨てworktree内で完結し、呼び出し元の
作業ツリーは変更しない。`docs-consistency.yml`が毎push実行）。**所要は検知器の数×2経路ぶん
`docs`を子プロセスで回す構造のため、検知器を足すと線形に伸びる**。1プローブは`--only`で
その検知器だけを回すので、伸びるのは足した検知器のぶんだけになっている
（[T880](../../../docs/records/tasks/T880.md)）。
検知器は鳴らなくなっても静かに0件を出し続けるため、**「検知器がある」ことと「鳴る」ことは分けて確かめる**。
検知器を足したときはその検知器だけが拾う違反の作り方を`guard_probe_mutations`へ1件足す
（足さないとテストが落ちる）。

**鳴ることと、取りこぼしていないことも別物である。** 違反を母集団の内側へ置く正例は、
母集団が狭すぎることを原理的に検出できない。`mutate`は同じ実行で**母集団の外縁**
（`guard_probe_edges`）も試す——「拾うべきなのに今の母集団へ入らない位置」を検知器ごとに
1件ずつ置き、拾えるかどうかの**期待値**と突き合わせる。期待値がFalseの行は既知の穴
（`GAP`）で、穴を埋めたら期待値をTrueへ更新する。埋まったのに更新していない場合も、
埋めたはずの穴がまた空いた場合も`CHANGED`として落ちる。外縁を持たない検知器
（母集団が全件の場合）は、理由の文字列を代わりに置く。

**期待値がTrueの外縁は、一度は見逃したという観測を根拠に要求する。** 外縁のつもりで
母集団の**内側**へ違反を書いてしまうと初日から検知され、穴が埋まっているのと見分けが
つかなくなる——レビューが「取りこぼしていない」と言う根拠が、穴の有無と同じ見た目になる。
`mutate --update`が観測した`GAP`を`history/guard_edges.json`へ記録し、記録の無い
期待値Trueは`UNPROVEN`として落とす。記録へ先回りしてキーを書く抜け道は、実在する外縁の
ものだけを許す制約で塞いである（どちらも`test_review_checks.py`が毎CIで見る）。

**期待値がFalseの外縁は、同じ違反を母集団の内側へ置いた対照（`control`）を要求する。**
見逃されたという事実だけでは、位置が外だからなのか違反がそもそも成立していないのかを
区別できない。`mutate`は内側でも鳴らない外縁を`UNREAL`として落とす。**外縁と対照の本文は
1つの素材から組み立てる**——別々に書くと、位置以外の違い（綴り・記法のずれ）が混じっても
「外だから見逃されている」と読めてしまう。
...
- **トリガー**: 次のいずれか早い方に達したら`/review:all`（最低限`/review:consistency`）を
  実施する。①前回いずれかの種別のレビュー実行（`history/`の最新ファイル日付）から**14日経過**、
  ②直近の`all`/`consistency`/`overall`の対象コミットから現在までの**実装コード
  （`backend/app`・`frontend/src`、テスト・生成物除く）の変更行数が20,000行以上**
  （挿入＋削除）。③**量に関係なく**、T400のような分割元タスク（複数のTxxxへ分割する
  規模Lのタスク）が完了した直後は必ず実施する（大きな設計変更の直後こそドリフトが
  起きやすいため）。閾値の根拠は[_history.md](_history.md)「2026-09-03: 周期レビューの
  負荷軽減」参照。
- 新しいセッションが作業に着手する際は、`python scripts/review_checks.py trigger`で①②に
  該当していないか確認する（③は自動判定できないため自分で判断する。CLAUDE.mdからこの節への
  ポインタあり）。
