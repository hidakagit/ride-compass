...
- **判断・実行を保留する場合、「後で判断」等の一言メモで済ませず、影響範囲
  ...
  一緒に書く。
- **分割元タスク（T400のように複数のTxxxへ分割するタスク）を完了とする前に、
  分割した子タスクのdocs/tasks/Txxx.mdすべてを「未起票」「別タスク」「スコープ外」等の
  語で機械的に検索し、該当箇所を洗い出す。各該当箇所は、①起票済みタスク番号にリンクされて
  いるか、②起票しないと決めた理由が明記されているか、のいずれかである状態にしてから
  分割元を完了とする**（2026-08-30、T405が「環境グループの風[時刻＋方向指定→gridFill面表示]
  はT400.md記載の別タスク・未起票・スコープ外」と正しくフラグを立てていたにもかかわらず、
  T400自体を完了扱いにする際にこのフラグが実際に起票されたか確認されず、タスク番号を
  振られないまま見過ごされた実績を受けて明文化。個々のタスクが正しくフラグを立てても、
  親タスクを閉じる工程がそれを拾わなければ同じ穴が残る。T405のように**バックグラウンド
  実行のタスクはユーザーが逐次内容を確認できないため、この種の見落としに気づく機会が
  ユーザー側に無く、実質的に握りつぶした判断になってしまう**——このチェックはバック
  グラウンド実行時こそ重要。上記直前の「保留の完全記録」ルールが起票する瞬間の話なのに
  対し、このルールは分割元タスクを閉じる瞬間に見落としを回収する話で、両者は対になる。
