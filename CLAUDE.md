...
古くなった記述）を見つけたら同一コミットで修正すること**（2026-08-31、モジュール
設計書の全ソース再検証セッションで多数の乖離が見つかった実績を受け厳命化。サンプリング
読みで済ませないこと自体は本ファイル全体の運用と地続き）。
...
- **判断・実行を保留する場合、「後で判断」等の一言メモで済ませず、影響範囲
  ...
  一緒に書く。
- **どのタスクを完了扱いにする前にも、以下4つの機械的チェックをその場で行う**（安価で
  ...
  3. **docs/modules/\*.mdを変更した場合**: `docs/modules/README.md`「記載粒度」節が禁止する
     ...
     Txxxへの違反リンクが変更箇所に紛れていないか見直す（2026-08-31、T458/T459対応中に
     自分自身がこの禁止パターンへ違反する記述を追加してしまい、ユーザー指摘で発覚・全15
     ファイル監査で他の既存違反も含め是正した実績を受けて追加。既存モジュールを修正する
     際に対象docsを精読するだけでなく、自分が新たに書き加える記述自体もこのルールに
     従っているか確認すること）。
  4. **新規ファイル（コンポーネント・フック・サービス・ルーター等）を追加した場合**:
     ...
     コミットメッセージで実装内容を列挙する作業の中では見落としやすい（2026-09-01、
     T517完了コミットが実装で直接変更した既存メカニズムの説明箇所[`axis-studio.md`の
     `proxyToBackendAdmin`記述・`cross-cutting-infrastructure.md`のAPI仕様]は追従
     したが、新設した`BackendLogsPanel.tsx`自体の責務を持つはずの
     `developer-research-tools.md`の対象ファイル表への追記が漏れ、ユーザー指摘で
     発覚した実績を受けて追加。原因は「コードを触った箇所＝ドキュメントの更新対象」という
     ファイル追跡ベースの判断だけで済ませ、`docs/modules/README.md`索引を引いて
     「この新規ファイルの責務はどのモジュールが持つか」を逆引きする一手間を踏んで
     いなかったこと）。このチェック自体の実施漏れ（個別タスクの自己申告依存）を補う
     二重チェックと、対になる欠陥である死んだ参照（削除済みファイルへの言及が
