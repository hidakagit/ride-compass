...
| **.claude/commands/review/*.md**（`/review:consistency`等） | **完了後に検証するチェック項目**（設計・実装・テストの乖離検出等） | 実装**中**に常に意識すべきものではなく、実装が終わった後に「ちゃんとできているか」を確認する性質のもの。既存のレビュー基盤（[README](.claude/commands/review/README.md)参照）が対象 |
...
- **判断・実行を保留する場合、「後で判断」等の一言メモで済ませず、影響範囲
  ...
  一緒に書く。
- **分割元タスク（T400のように複数のTxxxへ分割するタスク）を完了とする前に、子タスクの
  未起票フォローアップが残っていないか確認する**——これは完了後の検証チェックのため
  詳細・手順は`.claude/commands/review/consistency.md`（「設計 ↔ 実装」節）へ2026-08-30に
  移設した。分割元タスクを完了扱いにする際は`/review:consistency`の観点で自己チェック
  するか、同レビューを実行すること。
