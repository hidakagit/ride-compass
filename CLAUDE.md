# RideCompass

サイクリング向け周回ルート生成アプリ。backend（FastAPI）+ frontend（Next.js）。
アーキテクチャ全体は docs/architecture.md 参照。

設計レビュー（2026-08-15〜16）の指摘と改善実行計画は docs/improvement-plan.md にある。
リファクタリング・機能追加の着手前に該当タスクの有無を確認し、完了したらチェックを更新すること。
improvement-plan.md自体はチェックボックス付きリンクの一覧（インデックス）のみを持ち、
各タスクの背景・対応方針・実装メモ・検証結果はdocs/tasks/Txxx.md（タスク番号1件=1ファイル）
にある。
設計原則（RideCompass固有の仕様）は docs/design-principles.md が唯一の正本（常に最新）。
新しい仕組みを作る前は必ず読むこと。判断原則・進め方は
.claude/commands/review/principles.md「判断原則」節、機能単位の設計は
docs/modules/README.md（backend8・frontend7の全15モジュール、実コードのみを根拠に記述）
を参照。**改善計画のタスクに限らず、既存モジュールへの修正・追加に着手する際は、
対象となるdocs/modules/*.mdを必ず先に精読し、記載と実装の乖離（未記載のファイル・
古くなった記述）を見つけたら同一コミットで修正すること**（サンプリング読みで済ませない
こと自体は本ファイル全体の運用と地続き）。
タスクの一生（着手前〜実装〜完了〜周期レビュー）を通しで見たい場合は docs/workflow.md 参照
（個々のルールの本文はそちらに複製せず、どの段階で何を参照するかだけを示す）。

## ドキュメント階層（必読、新しいルールを追加する前に確認すること）

ルール・方針を書く場所は以下の基準で決める。**何でもCLAUDE.mdへ足さない**——CLAUDE.mdは
毎回のセッション開始時に無条件で全文読み込まれるため、肥大化すると本当に常時必要な内容が
埋もれる。

| 階層 | 置くもの | 判定基準 |
|---|---|---|
| **CLAUDE.md（本ファイル）** | 常時・無条件に従うべき短い方針＋詳細docsへのポインタ | 「毎回のタスクで、明示的に呼ばれなくても常に適用すべきか」がYesかつ短く書けるもの。長くなる場合は要点だけ残し詳細を下記のdocs/へ追い出す。**ルール自体の経緯（いつ・何が起きて追加したか）はここに書かず、該当するdocs/tasks/Txxx.mdへ任せる**——本ファイルは毎セッション無条件で全文読み込まれるため、経緯の蓄積がそのまま肥大化に直結する |
| **docs/&lt;topic&gt;.md**（logging.md・testing.md等） | トピック別の詳細方針・経緯・具体例 | CLAUDE.mdからは要点＋「詳細は◯◯.md参照」の形で参照するだけにする |
| **.claude/commands/review/*.md**（`/review:consistency`等） | **複数の変更を並べて初めて判断できる、周期実行向きのチェック**（複雑度のトレンド・UI一貫性・アーキテクチャ全体の一貫性等） | 1タスク単体では判断材料が無い・判断にコストがかかるもの。既存のレビュー基盤（[README](.claude/commands/review/README.md)参照）が対象。**注意**: grep一発で済むような安価で機械的なチェックはここへ寄せない——「次の周期レビューまで気づけない」設計になり手遅れになる。そういうものはCLAUDE.md側の「毎タスク完了時の即時チェック」に置き、ここには二重チェックとしてのみ残す |
| **docs/tasks/Txxx.md** | そのタスク限定の背景・決定・実装メモ | 他のタスクへ一般化しない個別事項。一般化できるなら上記いずれかへ昇格する |

## 出力言語（必読）

**ユーザーへ見せる結果・メッセージは常にすべて日本語で表示すること。** 対話中の通常の
返信本文（要約・説明・質問・見出し等）はもちろん、バックグラウンドで動くタスク
（Bash/PowerShellのrun_in_background・Agentのdescription・ScheduleWakeupのreason・
CronCreate等）に付随する進捗・ログ・通知メッセージも例外なく日本語にする。英語の技術用語
（ファイル名・コマンド・ライブラリ名等）はそのままでよいが、地の文・説明・見出しは日本語で書く。

## ログ方針（必読）

**コードを追加・変更するときは docs/logging.md のログ方針に従うこと。** 要点:

- エラー・429拒否・候補0件はWARNING以上で**常時**出す（debug_modeはDEBUG詳細の追加スイッチであり、エラー出力の条件にしない）
- 外部API/キャッシュアクセスは `app/infrastructure/debug_log.py` の `log_external_call` で囲む（cache hit/miss・result・statusをfieldsに設定。ログと /api/debug/stats の統計が自動で付く）
- 高コスト処理はステージ別所要時間と中間結果の減り方を1行INFOサマリにする（route_generator.py参照）
- リクエストIDは request_log.py のミドルウェアが全ログへ自動付与する。個別ログに書かない
- 常時出るログの座標は小数2桁へ丸める。APIキーはどのレベルでも出さない

## コメント方針（必読）

**コードコメントに経緯（いつ・誰の指摘で・改善計画Txxxで・以前は〜だった・実機/実測で
〜だったため等）を書かないこと。** 詳細な判定基準・実例はdocs/comments.md参照。要点:

- 書いてよいのは「今も成り立つ非自明な制約・理由」を現在形の一文で書いたものだけ
  （例:「このロックはXがYから並列に呼ばれるため必要」）。実装を読めば分かることは書かない。
- 経緯は書かない。同じ内容が対応する`docs/modules/*.md`に既にあるならコード側は削除する
  （二重メンテ防止）。経緯自体を残す価値があるなら該当タスクの`docs/tasks/Txxx.md`へ
  （新規に書き足すより、既にそこにある背景と重複していないか先に確認する）。
- 新規に書く/変更するコメントは直ちにこの方針へ従う。既存コードに残る経緯コメントの一掃は
  個別タスク（[T567](docs/tasks/T567.md)、モジュール単位で分割）として計画的に実施し、
  一括では行わない（背景・規模は[T566](docs/tasks/T566.md)参照）。
- 新規混入分はpre-commitフック・CIが機械的に検出しブロックする（[T570](docs/tasks/T570.md)、
  仕組みはdocs/comments.md「機械的な強制」参照）。自己申告のみに頼らないこと。

## ドキュメント記載方針（必読）

**ドキュメントに要素を数え上げないこと（個数・全件の一覧）。示すべきは挙動。**
詳細・実例はdocs/documentation.md参照。要点:

- 書かない: 個数（「4本のバッチ」「現行8軸」「5値の分類」）・全件の一覧（ファイル/
  エンドポイント/state/定数/軸id）。**要素が1つ増えたときに嘘になる文は数え上げている**
- 書く: その要素群が何をするか・増えたときに何が起きるか・実装を読んだだけでは
  分からないこと（暗黙の前提・外部システムの挙動・実測値）
- **代表例は許容する**。挙動を分かりやすくするためなら「◯◯等」「例:」の形で挙げてよい
- 全件が必要なら生成物（axis-catalog.json等）かそれを定義するコードを指す
- 例外は**機械が完全性を検査している表**だけ（現在はdocs/modules/*.mdの対象ファイル表。
  `find_undocumented_files`が検査する）。同じ一覧を検査の無い場所へ二重に持たない

## テスト方針（必読）

**新しいテストを追加するときは docs/testing.md のパターンに従うこと。** 要点:

- レート制限の境界値テストは `rate_limiter.check_rate_limit` を直接呼んで上限-1件を埋め、実HTTPは境界の1〜2回に絞る（上限回数分の実HTTPループ厳禁）
- PostGIS統合テスト（road_graph_session、conftest.py）はファイル単位でエンジン・イベントループを共有する設計。新規ファイルでは `pytestmark = pytest.mark.asyncio(loop_scope="module")` が必要（自前の追加async fixtureにも `loop_scope="module"` を明示）。CIはpytest-xdistで並列化しているため `pytest.mark.xdist_group(name="postgis")` も併せて必須（詳細はdocs/testing.md参照）
- フロントエンドの新規テストがDOM（render/renderHook/window等）を使わない純ロジックなら、ファイル先頭へ `// @vitest-environment node` docblockを付ける（実装側関数の隠れたDOM依存にも注意、詳細はdocs/testing.md参照）
- **修正→確認を繰り返す反復フェーズでは、変更に直接関係するテストファイルだけを絞り込んで
  実行する**（backend例: `pytest backend/tests/test_foo.py -q`、frontend例:
  `npx vitest run <該当ファイル> --pool=threads`）。バックエンド全体
  （`pytest -q -m "not postgis"`・`ruff check app scripts benchmarks tests`）・
  フロントエンド全体（`npx vitest run`・`tsc --noEmit`・`eslint`）のような
  フルスイートは、**実装が安定し反復が収束した後の最終検証として1回だけ**実行する。毎回の微修正のたびにフルスイートを回すと、
  「微修正→長いフルテスト→NG発見→修正→また長いフルテスト」という非効率なループに陥る。

## キャッシュ方針（必読）

**キャッシュを新設・変更するときは docs/caching.md に従うこと。** 要点:

- 保持層は「共有範囲×寿命」の2軸で選ぶ。**Redisへ置くのは外部への負荷を肩代わりするものだけ**
  （失っても自前のPostGISから復元できるものはプロセス内かディスクへ置く）
- プロセス内キャッシュは`cachetools`（TTLCache/LRUCache）に統一する。追い出しを自前で書かない
- Redisへ持つときは`redis_json_cache.py`の`get_json`/`set_json`を使う（骨格を写経しない。
  pre-commitとCIが機械的に検出する）
- 「判断」（鮮度・真偽値）をキャッシュしてよいのは、入力が全てキーにあり・単調かつ安全側で・
  TTLが許容窓から導かれているときだけ
- ディスクへ世代番号を使うなら、旧世代を削除する導線をセットで用意する（世代番号はディスクの
  古い実体を消さない）

## コミット時の同期ルール（必読）

コード変更と同期して更新すべきペアの漏れが繰り返し発生している。以下は**同一コミットで**
必ず実施すること。

- **backend側のAPIルーター・Pydanticモデル・レジストリ・domain定数を変更したら**、
  `backend/scripts/export_openapi.py`→`cd frontend && npm run generate:api`を実行し、
  `git diff --exit-code -- frontend/src/types/generated/`がクリーンであることを確認する。
- **規模M以上でAPI・ドメイン概念・レイヤー種を新設するタスクは、完了条件へ
  docs/architecture.md追従を既定で含める**。docs（「現状」記述）はコード変更と
  同一コミットで更新する。
- **既存の仕組みと技術的に別方式の新しい配信・レンダリング機構（例: タイル焼き込み済み
  ramp軸に対する`dedicated_way_value_layer`のようなRedis経由way_id配信）を新設するときは、
  着手前に`docs/design-principles.md`の構造仕様3・8（1本道の追加点）がこの新しい機構にも
  適用されるかを点検し、適用されるなら軸ごとのファイル・関数・定数・propを新設しない
  汎用設計にする**（新しい種類の機構を作る時にだけ点検が漏れやすい）。
- **MVT焼き込み値（CASE式・材料タグ・domain純関数）を変更したら**、対応するタイル世代
  定数（`ROAD_SURFACE_TILE_VERSION`等）と生成物（region-tile-config.json）を同一コミットで
  上げる。
- **評価軸（`axis_definitions`テーブル）の新規追加・削除・既存軸の`shape_params`調整は、
  すべて`axis_admin`のAPI（軸スタジオのGUI、または直接API呼び出し。新規追加=POST、
  削除=unpublish→DELETE、公開軸の調整=unpublish→PUT→republish）経由で行う。
  `backend/migrations/`は`axis_definitions`/`axis_registry_meta`の**テーブル構造
  （DDL）のみ**を管理し、以後の新規migrationへ軸の行データ（INSERT/UPDATE/DELETE）を
  追加することはない**（経緯は[T361](docs/tasks/T361.md)参照）。0014〜0022（過去に追加した行データ入りの
  migration）はこのプロジェクトの標準運用どおり書き換えない。fresh bootstrap（CI・
  新規環境・disaster recovery、まっさらなDBへ`create_tables()`→
  `apply_pending_migrations()`を適用した直後）は、`backend/fixtures/
  axis_definitions_snapshot.json`（現在の実DBの内容を`backend/scripts/
  dump_axis_definitions_snapshot.py`でダンプしたスナップショット）を`backend/scripts/
  bootstrap_fresh_db.py`（新規環境用）・`backend/scripts/bootstrap_ci_db.py`（CI用）が
  読み込み、テーブルを丸ごと置き換えて実データを用意する
  （`app/infrastructure/axis_definitions_snapshot.py: load_axis_definitions_snapshot`）。
  このロードは無条件にテーブルを空にしてから投入するfresh bootstrap専用の操作のため、
  通常のアプリ起動経路（`main.py`のlifespan・`refresh_axis_definitions`）や、稼働中の
  DBに対して繰り返し実行される`app/batch/import_pbf.py`等からは呼ばない（誤って本番の
  生きた軸データをスナップショットの内容で上書きする事故を防ぐため）。スナップショットの
  更新は手動運用（本番/devでAPI経由の軸変更を行った後、`dump_axis_definitions_snapshot.py`
  を都度手動実行してリフレッシュしコミットする。自動化はしていない——低頻度な操作の
  ためデプロイパイプラインへ組み込むリスクの方が上回ると判断した）。
  `domain/axis_definitions.py: AXIS_DEFINITIONS`のPython literalは撤去済みでDBが
  唯一の正本。`tests/test_migrate.py`のブートストラップテストは、
  まっさらなDBへ全migration適用→スナップショット読み込みまでの一連の流れを検証する
  （DB接続が要るため`pytest -m "not postgis"`実行時はスキップされる。ローカル
  PostgreSQLを起動して`pytest tests/test_migrate.py`を実行し確認すること）。
- **axis_admin API経由のDB行データ変更を完了扱いにする条件は[docs/deployment-sync.md]
  (docs/deployment-sync.md)参照**（開発DBのみの反映で完了扱いにしない）。
- **既存DBの行データを新しいコードが読めなくなる変更（Pydanticモデルの破壊的変更等）を
  含む`backend/**`の変更は、本番DBのデータ移行を完了させてからpushする**。
  `.github/workflows/deploy-backend.yml`は`push`かつ`backend/**`パスの変更を検知すると
  本番へ自動デプロイする。DB移行より先にpushすると、新コードが本番DBに残る旧形式データを
  読めず、`refresh_axis_definitions`等のfail-fast設計により本番backendが起動失敗する
  （本番障害の実績あり、詳細は[T396](docs/tasks/T396.md)参照）。
  対応順序は事前に次のいずれかを選ぶこと: 1) 本番DBのデータ移行を先に完了させてから
  push、2) 移行を即座に行えない場合は`workflow_dispatch`のみで手動デプロイへ切り替える、
  3) 新旧両方の形式を一時的に許容する後方互換コードを経由して段階的に移行する。
- **規模M以上の変更は、着手前の最初のコミットで対応するタスクエントリを先に作成する**。
  作業内容が変わりうる大きめのタスクほど、途中で背景・方針判断が失われやすく、後から
  遡って記録を作ると経緯（なぜその設計にしたか）が失われるため。
  `docs/improvement-plan.md`は節見出し＋チェックボックス付きリンクの一覧のみを持ち、
  個々のタスクの背景・対応方針・実装メモは`docs/tasks/Txxx.md`へ分離されている:
  `docs/tasks/Txxx.md`を新規作成して詳細を書き、`docs/improvement-plan.md`の該当節へ
  `- [ ] [Txxx](tasks/Txxx.md). タイトル 規模...`の1行を追記する。番号の採番・衝突時の
  振り直し手順は次の「作業ツリーの安全」節を参照。
- **判断・実行を保留する場合、「後で判断」等の一言メモで済ませず、影響範囲
  （保留することで何がブロックされるか・何が動かなくなるか）を明記した完全な
  タスクエントリとして起票する**。保留の理由だけでなく、保留し続けた場合に次に
  何が起きうるかを一緒に書く。
- **どのタスクを完了扱いにする前にも、以下6つの機械的チェックをその場で行う**（安価で
  判断を要さないため、周期レビュー[`/review:all`等]まで待たない——周期レビュー側だけに
  置くと次のレビューが来るまで気づけない。詳細な書式は`.claude/commands/review/
  consistency.md`「設計 ↔ 実装」節にも二重チェックとして残してある）:
  1. **分割元タスク（T400のように複数のTxxxへ分割するタスク）の場合**: 分割した子タスクの
     docs/tasks/Txxx.mdすべてを「未起票」「別タスク」「スコープ外」等の語で機械的に検索し、
     各該当箇所が①起票済みタスク番号にリンクされているか②起票しない理由が明記されているか
     のいずれかであることを確認する。
  2. **既存の共有フィールド・パターンを変更/廃止する場合**: 直接触ったファイルだけでなく、
     変更前のフィールド・パターンをリポジトリ全体でgrepし、①他に取り残された消費者が
     いないか（詳細は[T401](docs/tasks/T401.md)参照）、②**残る消費者が変更後の契約と
     両立するか**——従来その契約が無いことを前提に外側から別の値を与えていた箇所は、
     直した側が正しく動くぶんだけその指定が静かに巻き戻る——の両方を確認する。
  3. **docs/modules/\*.mdを変更した場合**: `docs/modules/README.md`「記載粒度」節が禁止する
     経緯記述（「以前は…」「改善計画Txxxで…に変更した」「実機/実測で…だったため」等）・
     Txxxへの違反リンクが変更箇所に紛れていないか見直す。既存モジュールを修正する際に
     対象docsを精読するだけでなく、自分が新たに書き加える記述自体もこのルールに
     従っているか確認すること。
  4. **新規ファイル（コンポーネント・フック・サービス・ルーター等）を追加した場合**:
     そのファイルパス（またはファイル名）を`docs/modules/*.md`全体でgrepし、
     どこにも出現しなければ、`docs/modules/README.md`索引の「内容」列の1行説明を根拠に
     責務が一致するモジュールを特定して対象ファイル表へ追記する（複数モジュールに
     またがる場合は主たる責務側の1箇所でよい）。**「変更中に既存の記述へ矛盾が
     生まれていないか」（チェック3・変更前提のdiffベースの確認）だけでは、新規追加
     ファイルは検知できない**——新規ファイルは元々どの行とも矛盾しようがないため、
     コミットメッセージで実装内容を列挙する作業の中では見落としやすい（原因は「コードを
     触った箇所＝ドキュメントの更新対象」というファイル追跡ベースの判断だけで済ませ、
     `docs/modules/README.md`索引を引いて「この新規ファイルの責務はどのモジュールが
     持つか」を逆引きする一手間を踏んでいないこと。詳細は[T517](docs/tasks/T517.md)参照）。
     このチェック自体の実施漏れ（個別タスクの自己申告依存）を補う二重チェックと、対になる
     欠陥である死んだ参照（削除済みファイルへの言及がdocs/modules側に残ること）の検出は
     `.claude/commands/review/consistency.md`「設計 ↔ 実装」節に持たせてある。
  5. **自分が新規に書いた/変更したソースコードのコメントに経緯記述が紛れていないか
     見直す**（「以前は…」「改善計画Txxxで…」「ユーザー(要望|指摘|判断)…」「実機
     フィードバック…」等、docs/comments.md参照）。含む場合は現在形の制約表現へ書き換えるか、
     対応する`docs/modules/*.md`に同内容が既にあるなら削除する（背景は
     [T566](docs/tasks/T566.md)参照。チェック3が「docs/modules/\*.mdを変更した場合」の
     経緯記述を対象にするのに対し、これはソースコード全般のコメントが対象という違いがある）。
  6. **単一タスクエントリ内に複数の段階・対応方針項目があり、そのうち一部だけを完了させて
     `[x]`化する場合**: 残っている段階・項目のうち、「見送り」（今後もやらない確定判断）
     ではなく「まだ実施予定・トリガー待ち」のものは、元タスクの本文に埋め込んだままにせず、
     `docs/improvement-plan.md`へ独立した`- [ ]`行として別Txxxへ起票する（`/task:next`等の
     機械抽出は`- [ ]`行のみを対象にするため、親タスクを`[x]`化した時点でその中に埋もれた
     残項目は以後手動で思い出さない限り拾われなくなる。2026-09-04、[T281](docs/tasks/T281.md)
     の段階3[鮮度台帳]を段階1・2の完了を理由に本文へ残したまま`[x]`化しようとし、
     ユーザー指摘で発覚・[T571](docs/tasks/T571.md)へ分離した実績を受けて追加）。
- **作業に着手する際、`.claude/commands/review/README.md`「定期的なレビュー」節の閾値
  （前回レビューから14日 or 実装コードの変更20,000行、どちらか早い方。または分割元タスク
  完了直後。`python scripts/review_checks.py trigger`で判定）に該当していないか確認し、
  該当する場合は`/review:all`実施と`/code-review`実施提案を行う**
  （詳細・code-review結果の蓄積経路は同README.md参照）。

## 作業ツリーの安全（必読）

このプロジェクトは並行セッション（複数のClaude Codeセッション・エージェント）が同じ作業ツリーを
同時に触りうる。以下を必ず守ること。

- **自分が変更したのではないファイルの変更を作業ツリーで見つけても、絶対に自動で戻さない**
  （`git checkout --`/`git reset`/`git stash`/`git clean`等）。「明らかに別エージェントの
  誤操作に見える」場合でも、まず並行セッションが作業中である可能性を疑う。対応が必要と
  思われる場合は、変更内容を報告してユーザーに確認してから行う。
  `git checkout --`等で破棄した未ステージの変更は、gitオブジェクトとして残らないため
  多くの場合復元不能である。
- **最初から変更を目的としていない作業（調査・分析・レビュー系コマンド等）は、本体の
  作業ツリーではなく専用の `git worktree` で実施する**。読み取り専用の調査であっても、
  本体の作業ツリーを一切変更しない構成にすることで、並行セッションとの衝突・誤操作の
  リスクそのものを無くす。
- **バックグラウンドAgentへ実装タスクを委任するときは、`isolation: "worktree"`を必ず
  指定する**（isolationを指定しないと自分自身の並行作業やユーザーの手元の変更と衝突・
  混在するリスクがあり、「並行セッションが同じ作業ツリーを触りうる」という本節の前提が
  自分のAgent委任にもそのまま当てはまる。詳細は[T414](docs/tasks/T414.md)参照）。
- **improvement-plan.mdのタスク番号（Txxx）は、複数セッションが同時に「次の番号」を
  独立に計算するため衝突する**（複数回実績あり）。番号を決めてから実装を終えてコミットする
  までの時間が長いほど、その間に他セッションが同じ番号を使って先にpushしてしまう
  リスクが上がる。この時間差を縮めるため:
  1. 新規タスクの着手を決めたら、**`docs/tasks/Txxx.md`に背景1〜2行だけの最小限の
     スタブを作り、`docs/improvement-plan.md`の該当節へインデックス行を1本足した
     時点ですぐコミット・push**する（`git fetch origin master`で直前の状態を確認して
     から採番する）。対応方針・実装メモ・検証結果などの詳細は、push成功を確認した後の
     追加コミットで`docs/tasks/Txxx.md`へ書き足せばよい——「実装を終えてからまとめて
     起票」ではなく「番号だけ即確保してから中身を育てる」順にすることで、衝突しうる
     時間窓を実装時間全体から「fetch→2ファイルに1行ずつ書く→push」の数秒〜数十秒へ
     縮める。
  2. pushしようとして、または起票後の再fetchで、自分の番号が既に他セッションに
     使われていたと分かったら、**自分の側を次の空き番号へ振り直す**（既にmaster側へ
     載っている方はそのまま触らない）。見出しだけでなく、コード側のコメント
     （`# 改善計画Txxx`等）も含めて旧番号への参照を全てgrepし揃えて置換すること
     （置換漏れでコメントとドキュメントの番号がずれた実績がある）。振り直した経緯
     （旧番号・理由）をエントリ内に一言残す。
  3. 完了報告としてmasterへ最終pushする直前にも`git fetch origin master`をもう一度行い、
     その時点で採番が競合していないか確認する（作業完了ごとにmasterへ反映・次作業前に
     fetchする、という本ファイル全体の運用と地続き）。
