...
- **どのタスクを完了扱いにする前にも、以下3つの機械的チェックをその場で行う**（安価で
  ...
  3. **「N個の◯◯」「両方の△△」のように個数・組み合わせを主張する記述が変わる変更
     （例: エンジンが2つ→1つ、選択肢がN個→N-1個）の場合**: 削除した具体的シンボル名
     （関数名・設定名等）だけを逆引きgrepするのでは不十分——「両エンジン」「どちらか」
     「2つの」等、個数に依存するナラティブ的表現そのものをコードのコメント/docstringと
     docs/modules/\*.mdの両方からgrepし、事実が変わった後も同じ主張が残っていないか
     確認する（2026-08-31、T462がopenrouteserviceエンジンを削除した際、削除した
     具体的シンボル名の逆引きgrepだけでは「両エンジンが〜」という個数依存の言い回しを
     検出できず、code-reviewとユーザー指摘で計6箇所[コード5・docs/modules 1]の
     見落としが後から発覚した）。
