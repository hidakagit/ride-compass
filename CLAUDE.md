...
実現不能な方針のタスクができる。[T722](docs/tasks/T722.md)）。領域と正本の対応は
...
- **テストが落ちたら、①失敗対象を最小コストで洗い出す→②失敗の原因を取る→③同根を
  ...
  （詳細はdocs/testing.md「テストが落ちたときの直し方」参照）。**①の出力は「何が落ちたか」
...
- **修正→確認を繰り返す反復フェーズでは、変更に直接関係するテストファイルだけを絞り込んで
  ...
  **backendは`-m postgis`も完了前に1回通す**（手元の既定実行から外れ、CIだけが実行するため。開発機での並列実行の可否・所要時間はdocs/testing.md「開発機でのbackendテストの回し方」参照）。
- **影響範囲が自分でも分からないときは、範囲を導出してから絞る**。シンボルの撤去・改名の
  ...
  **`pytest backend/tests -q --co` （収集のみ、数秒）でimportが壊れたファイルを出し、
  ...
  フルスイートを「影響範囲の調査」に使わない——最終検証のためのものであり、調査に使うと
  1回あたり数分を払いながら、どのファイルを直すべきかは結局出力から読み直すことになる。
- **コードの構造について何か言う前に、機械的に列挙する**。「◯◯は2か所にある」「これが
  ...
  （[T956](docs/tasks/T956.md)）。既に同じ目的の仕組みがあることにも、そこで初めて気づいた。
...
そのテストまで一緒に保守対象として残り続ける（実例: [T240](docs/tasks/T240.md)で
`compute_edge_cost`を「回帰テストのオラクル」として残すと着手時の方針に書き、
T536でそれを置き換えた`compute_edge_costs_bulk`まで同じ理由で残した。
[T956](docs/tasks/T956.md)で両方とそのテストを削除するまで1か月近く残骸だった）。
...
そのものであり、文章のルールは足さない）。**ただし検知器を1本足す前に、既存の検知器と
母集団が重なっていないか・これは既存の不備なのか新規の問題なのかを確かめる**——思い付きは
トリガーとしては正しいが、全体と突き合わせずに足すと重複を生産する（[T966](docs/tasks/T966.md)）。検知器にできないものだけを下記へ置き、その場合も
**実行したコマンドと観測値をコミットメッセージへ書く**（「確認した」とだけ書かない）。
上の後半3項目は、検知器の候補を実測したうえで届かないと判定して置いたもの
（数字と判定は[T798](docs/tasks/T798.md)）。
...
- **worktreeはタスク単位で作り捨てず、テーマ単位で使い回す**。frontendの依存は実体が
  ...
  リンクの中へ入り共有元を壊す（実測、[T768](docs/tasks/T768.md)）。
