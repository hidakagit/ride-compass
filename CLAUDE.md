...
- **判断・実行を保留する場合、「後で判断」等の一言メモで済ませず、影響範囲
  ...
  一緒に書く。
- **どのタスクを完了扱いにする前にも、以下3つの機械的チェックをその場で行う**（安価で
  ...
  3. **docs/design-principles.md・docs/architecture.md・docs/modules/\*.mdを編集した場合**:
     追記・修正した文中に「以前は」「Txxxで解消/是正/撤去した」「〜という理由で変更した」等、
     経緯・変更理由を語る表現が紛れ込んでいないかその場でgrepし、見つかったら現状の記述
     だけへ書き直す（経緯はdocs/tasks/Txxx.md側へ譲る。判断原則15「docsは『現状』と
     『経緯』を分ける」）。architecture.mdに既に蓄積している同種の記述はT428
     （経緯記述のdecisions/への抽出）が別途担当しており、本チェックの目的は
     「新規に増やさない」ことに限る（2026-08-31、T473実施中に複雑度平衡性レビューの
     成果[design-principles.md新設の経緯・軸専用propの是正理由]をそのまま
     docs/modules/frontend/\*.md・design-principles.mdへ転記してしまい、ユーザー指摘で
     発覚した実績を受けて追加）。
