...
# Evaluation Engineのオーケストレーション（仕様書26章）。I/Oは行わない。属性の取得自体は
# `ElevationAttributeService`・`GraphService.get_or_build_graph_with_attributes`が担当し、
# ここでは既に取得済みのRoadGraph・属性からEdge Costを算出するのみ。
#
# 探索コスト算出の既定経路は`RoadGraphEngine`のタイル単位静的スコア行列
# （`domain/evaluation.py: build_static_edge_score_matrix`）でこの関数を経由しない——
# `evaluate_graph`はテスト・ベンチマーク（`compute_edge_costs_bulk`との出力一致を確認する
# 回帰オラクル）向けに残る。状態を持たないためクラスにはしない。
