...
from app.domain.axis_definitions import invert_axis_breakpoints
...
class OpenRouteServiceEngine:
    ...
    def _build_segment_details(
        ...
        for i in range(len(points) - 1):
            ...
            # 公開軸car_stressのdifficulty(0-100)をinvert_axis_breakpoints（改善計画T292、
            # road_graph_engine.pyと同じ導出。単一ソースの逆変換）で逆変換して求める。
            car_stress_difficulty = axis_difficulties.axes.get("car_stress")
            car_stress = (
                round(invert_axis_breakpoints("car_stress", car_stress_difficulty))
                if car_stress_difficulty is not None
                else None
            )
