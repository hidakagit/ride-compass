...
class RouteCandidate(StrictModel):
    ...
    # **フロントはこの印を見ない**——周回では一度も付かないため、候補一覧の中から
    # 所要時間が最小のものを自分で選ぶ（`lib/routeTabLabel.ts: fastestRouteId`）。
    # この印は「backendが基準線として実際に別途探索した候補」を指すもので、一覧から
    # 導いた最小とは意味が違う（探索の妥当性を見るときに要る）。
