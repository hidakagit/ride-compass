...
class RouteCandidate(StrictModel):
    ...
    # （目的地モードのみ。周回は目標距離が距離を決めるため常にFalse）。
    # **フロントでの使われ方は分かれる**。表示するラベル（「最速」の印）には使わない
    # ——周回では一度も付かないため、候補一覧の中から所要時間が最小のものを自分で選ぶ
    # （`fastestRouteId`）。一方、**合成候補を差し込む位置の判定には使う**
    # （`insertByDifficulty`。先頭固定の基準線は難易度順の外にあるため、その後ろから
    # 挿入位置を探す）。この印は「backendが基準線として実際に別途探索した候補」を指すもので、
    # 一覧から導いた最小とは意味が違う（探索の妥当性を見るときに要る）。
