"""国土地理院タイルの製品ごとの事実（配信元が実データを持つ範囲・上流のパス・出典表記）。

**画面はこれを写さない。** 生成物（`region-tile-config.json`）へ書き出し、frontendはそれを
読む——写しを持つと、配信元の上限だけを広げたときに画面が要求せず、逆に画面だけを広げると
範囲外を要求し続ける（どちらも例外にならず、タイルが出ないだけになる）。

出典表記は利用条件の一部であり、データを取りに行く側が持つ。
"""
