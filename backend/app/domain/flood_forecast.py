"""JMA指定河川洪水予報の警戒レベル判定（改善計画T212）。

コード対応表は気象庁「指定河川洪水予報」電文フォーマット解説資料の表２
（令和8年度出水期以降の対応表、2026-08-22にdmdata.jp経由で取得）を典拠とする。
ライブデータ（2026-08-22、神田川のレベル4氾濫危険警報・善福寺川のレベル2氾濫注意報解除）
と突き合わせて確認済み。

JMA警報（T205、jma_warning.py）と異なり、このAPIはstatus文字列（"発表"/"継続"/"解除"）
...
# WarningBadgeの4段階（T174のWBGTと同じ語彙、advisory/warning/severe_warning/emergency_warning）
# へレベル2〜5をそのまま対応させる。JMA警報（T205）の3段階とは異なる軸だが、バッジの見た目の
# 語彙は共有できる（severe_warningはT174で既に追加済み）。
