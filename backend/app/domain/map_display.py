...
#: 軸が`map_value_thresholds`を宣言していないときの、地図が塗る段の境界。
#: **境界の値そのものではなく個数が段の数を決める**——増やせば凡例の行も増える。
DEFAULT_DIFFICULTY_BOUNDARIES: tuple[float, ...] = (33, 66)
#: 符号付き材料（勾配）の既定。上り側は1%刻み——登坂は1%の差で体感が変わり、配信値も
#: 0.1%まで保持している。下り側は踏まずに済むぶん差が小さいので粗い。
SIGNED_MATERIAL_BOUNDARIES: tuple[float, ...] = (-10, -5, -1, 1, 2, 3, 4, 5, 6, 7, 8, 10, 13)
