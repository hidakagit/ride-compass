...
# 実機で発生した。ルート生成（GENERATE_MAX_CONCURRENT）と同じ考え方で同時実行数を制限する。
# 現在はMVT生成をPostGIS側（ST_AsMVT、road_graph_repository.py）へ移して1タイルの
# コストを大幅に下げたが、キャッシュミスのバーストが遠隔DB/Session poolerへ無制限に
# 並ぶのを防ぐ歯止めとして同時実行数制限は維持する。
#
