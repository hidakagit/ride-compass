...
async def test_empty_database_yields_no_stale_generations(road_graph_session):
    ...
    for generation in counts.generations:
        assert set(generation.latest_available.values()) == {None}
...
async def test_edge_attribute_counts_reflects_fresh_generation_when_run_ids_match_latest(
    ...
    assert edge_counts.latest_available["source_osm_import_run_id"] == osm_run_id
...
async def test_edge_attribute_counts_reflects_stale_generation_when_newer_osm_run_exists(
    ...
    assert edge_counts.latest_available["source_osm_import_run_id"] == new_osm_run_id
    assert new_osm_run_id > old_osm_run_id


async def test_poi_only_import_does_not_age_way_derived_tables(road_graph_repository, road_graph_session):
    """`--pois-only`の取込は、way由来の派生テーブルを古くしない。

    way・nodeを1行も書かずに成功行を残すため、`MAX(id) WHERE status='succeeded'`をそのまま
    高水位にすると6テーブルが一斉に陳腐化判定になる（本番で実際に起きた）。誤警報が常態化
    すると、本物の陳腐化を見落とす。**POIを数えるedge_attribute_countsは逆に本当に古くなる**
    ので、そちらは古い判定のままでなければならない。
    """
    edge_ids = await _seed_one_way_and_edges(road_graph_repository, road_graph_session)
    full_run_id = await _insert_import_run(
        road_graph_session, "osm_import_runs",
        "pbf_name, profile_hash, status, started_at, finished_at, way_count, node_count, poi_count",
        "'full.pbf', 'hash1', 'succeeded', now(), now(), 100, 200, 50",
    )
    for edge_id in edge_ids:
        await road_graph_session.execute(
            text(
                "INSERT INTO edge_attribute_counts (edge_id, accident_count, intersection_count, "
                "computed_at, source_osm_import_run_id, algorithm_version) "
                "VALUES (:edge_id, 0, 0, now(), :run_id, 'v1')"
            ),
            {"edge_id": edge_id, "run_id": full_run_id},
        )
    await road_graph_session.execute(
        text(
            "INSERT INTO way_landcover (osm_way_id, data_source, data_version, computed_at, "
            "source_osm_import_run_id, algorithm_version) "
            "VALUES (100, 'test', '2024', now(), :run_id, :version)"
        ),
        {"run_id": full_run_id, "version": LANDCOVER_ALGORITHM_VERSION},
    )
    await road_graph_session.commit()

    # POIだけを取り込んだ成功run（way_count=0・node_count=0）。
    poi_only_run_id = await _insert_import_run(
        road_graph_session, "osm_import_runs",
        "pbf_name, profile_hash, status, started_at, finished_at, way_count, node_count, poi_count",
        "'pois.pbf', 'hash2', 'succeeded', now(), now(), 0, 0, 40",
    )
    await road_graph_session.commit()

    counts = await DerivedDataFreshnessQuery(road_graph_session).get_freshness_counts()
    landcover = next(g for g in counts.generations if g.table_name == "way_landcover")
    edge = next(g for g in counts.generations if g.table_name == "edge_attribute_counts")

    assert landcover.latest_available["source_osm_import_run_id"] == full_run_id
    assert edge.latest_available["source_osm_import_run_id"] == poi_only_run_id
    assert poi_only_run_id > full_run_id
