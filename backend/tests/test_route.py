...
def test_aggregate_segments_into_bins_single_edge_larger_than_bin_size_forms_its_own_bin():
    # 1本のEdgeがbin_distance_kmを超える長さのことがある。1本ずつ独立したビンになる
    # （分割はしない）。
...
def test_aggregate_segments_into_bins_axis_difficulties_distance_weighted_average():
    segments = [
        _segment(0, distance_km=0.3, axis_difficulties={"axis_a": 80.0}),
        _segment(1, distance_km=0.1, axis_difficulties={"axis_a": 20.0}),
    ]
    ...
    assert len(bins) == 1
    # (80*0.3 + 20*0.1) / 0.4 = 65.0
    assert bins[0].axis_difficulties["axis_a"] == pytest.approx(65.0)
...
def test_aggregate_segments_into_bins_axis_difficulties_averages_only_over_segments_that_have_it():
    ...
    # （持たない区間を0扱いで巻き込んで薄めてはならない）。軸を途中で非公開にすると
    # この形の入力になり、例外にもしてはならない。
    segments = [
        _segment(0, distance_km=0.2, axis_difficulties={"axis_a": 100.0, "axis_b": 40.0}),
        _segment(1, distance_km=0.2, axis_difficulties={"axis_a": 100.0}),
    ]

    bins = aggregate_segments_into_bins(segments, bin_distance_km=0.5)

    # axis_bが0扱いで平均されるなら(40*0.2+0*0.2)/0.4=20.0になってしまう。
    assert bins[0].axis_difficulties["axis_b"] == pytest.approx(40.0)
    assert bins[0].axis_difficulties["axis_a"] == pytest.approx(100.0)


def test_aggregate_segments_into_bins_axis_difficulties_omits_axis_absent_from_every_segment():
    # どの区間にも無いaxis_idは、結果の辞書にキー自体が現れない（「データ無しはキーを
    # 持たない」規約。frontendは値の有無だけで「出せる軸か」を判定できる）。
    segments = [
        _segment(0, distance_km=0.2, axis_difficulties={"axis_a": 50.0}),
        _segment(1, distance_km=0.2, axis_difficulties={"axis_a": 50.0}),
    ]

    bins = aggregate_segments_into_bins(segments, bin_distance_km=0.5)

    assert set(bins[0].axis_difficulties.keys()) == {"axis_a"}


def test_merge_material_values_distance_weighted_average():
    segments = [
        _segment(0, distance_km=0.3, material_values={"num_a": 8.0}),
        _segment(1, distance_km=0.1, material_values={"num_a": 4.0}),
    ]

    merged = merge_material_values(segments)

    # (8*0.3 + 4*0.1) / 0.4 = 7.0
    assert merged["num_a"] == pytest.approx(7.0)


def _dict_fields_of_segment() -> dict[str, type]:
    """`RouteSegmentDetail`の辞書フィールド名→値の型。

    値の型で絞らない——`dict[str, float]`だけを見ると、値が文字列の辞書フィールドが
    母集団から外れ、引き継がれていないことに気づけない。
    """
    out = {}
...
def test_aggregate_segments_into_bins_carries_every_dict_field():
    ...
    フィールド一覧をモデルから引いて機械的に検出する——足し忘れが型でも例外でも現れない。
    ...
    def probe(vtype, n):
        return {"probe": "val_x" if vtype is str else float(n)}
...
def test_axis_raw_values_round_to_significant_digits_not_fixed_decimals():
    # 生値は軸ごとにスケールが違う。固定の小数桁で丸めると桁の小さい軸で値がまるごと
    # 潰れ、0との区別が付かなくなるため、有効数字で丸めて相対精度を残す。
    small = aggregate_segments_into_bins(
        [_segment(0, distance_km=0.4, axis_raw_values={"axis_a": 0.0123456})], bin_distance_km=0.5
    )
    large = aggregate_segments_into_bins(
        [_segment(0, distance_km=0.4, axis_raw_values={"axis_a": 1234.5678})], bin_distance_km=0.5
    )

    assert small[0].axis_raw_values["axis_a"] == pytest.approx(0.01235)
    assert large[0].axis_raw_values["axis_a"] == pytest.approx(1235.0)


def test_axis_difficulties_still_round_to_one_decimal():
    # difficultyは0〜100の相対評価で、小数1桁より細かくしても判断は変わらない。
    segments = [_segment(0, distance_km=0.4, axis_difficulties={"axis_a": 12.3456})]

    bins = aggregate_segments_into_bins(segments, bin_distance_km=0.5)

    assert bins[0].axis_difficulties["axis_a"] == pytest.approx(12.3)


def test_merge_axis_raw_values_distance_weighted_average():
    segments = [
        _segment(0, distance_km=30.0, axis_raw_values={"axis_a": 0.5}),
        _segment(1, distance_km=10.0, axis_raw_values={"axis_a": 2.5}),
    ]

    merged = merge_axis_raw_values(segments)

    # (0.5*30 + 2.5*10) / 40 = 1.0。単位が「◯◯/km」の軸では、これへ距離を掛けた値が
    # 利用者へ出る。
    assert merged["axis_a"] == pytest.approx(1.0)


def test_merge_material_category_shares_is_distance_weighted():
    segments = [
        _segment(0, distance_km=6.0, material_categories={"cat_a": "val_x"}),
        _segment(1, distance_km=3.0, material_categories={"cat_a": "val_y"}),
        _segment(2, distance_km=1.0, material_categories={"cat_a": "val_x"}),
    ]

    shares = merge_material_category_shares(segments)

    assert shares["cat_a"] == {"val_x": 0.7, "val_y": 0.3}


def test_merge_material_category_shares_orders_by_share_descending():
    # フロントは並べ替えを持たず先頭を「最も延長の長い値」として出す。
    segments = [
        _segment(0, distance_km=1.0, material_categories={"cat_a": "val_x"}),
        _segment(1, distance_km=5.0, material_categories={"cat_a": "val_y"}),
    ]

    shares = merge_material_category_shares(segments)

    assert list(shares["cat_a"]) == ["val_y", "val_x"]


def test_merge_material_category_shares_excludes_segments_without_a_value():
    # 値の無い区間は分母にも入れない（「観測できた範囲でどの値が多いか」を表す）。
    segments = [
        _segment(0, distance_km=3.0, material_categories={"cat_a": "val_x"}),
        _segment(1, distance_km=7.0, material_categories={}),
    ]

    shares = merge_material_category_shares(segments)

    assert shares["cat_a"] == {"val_x": 1.0}
