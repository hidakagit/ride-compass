...
async def test_get_route_parses_surface_summary_when_present():
    feature = make_feature(distance_m=5000, duration_s=600)
    feature["properties"]["extras"] = {
        "surface": {"summary": [{"value": 1, "distance": 4000.0, "amount": 80.0}, {"value": 2, "distance": 1000.0, "amount": 20.0}]}
    }
    client = FakeORSClient(feature=feature)
    service = RoutingService(client)

    segment = await service.get_route([ORIGIN, DESTINATION])

    assert segment.surface_summary == [
        {"value": 1, "distance": 4000.0, "amount": 80.0},
        {"value": 2, "distance": 1000.0, "amount": 20.0},
    ]


async def test_get_route_surface_summary_is_none_when_absent():
    client = FakeORSClient(feature=make_feature(distance_m=5000, duration_s=600))
    service = RoutingService(client)

    segment = await service.get_route([ORIGIN, DESTINATION])

    assert segment.surface_summary is None


async def test_get_route_parses_surface_values_when_present():
    feature = make_feature(distance_m=5000, duration_s=600)
    feature["properties"]["extras"] = {"surface": {"values": [[0, 4, 1], [5, 9, 10]]}}
    client = FakeORSClient(feature=feature)
    service = RoutingService(client)

    segment = await service.get_route([ORIGIN, DESTINATION])

    assert segment.surface_values == [[0, 4, 1], [5, 9, 10]]


async def test_get_route_surface_values_is_none_when_absent():
    client = FakeORSClient(feature=make_feature(distance_m=5000, duration_s=600))
    service = RoutingService(client)

    segment = await service.get_route([ORIGIN, DESTINATION])

    assert segment.surface_values is None
