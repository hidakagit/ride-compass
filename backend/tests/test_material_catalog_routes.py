...
def test_get_material_catalog_includes_reference_points():
    # 軸スタジオの折れ点編集を助ける「値の目安」一覧。値を持つ材料（wind_penalty）は
    # 非空、値を持たない材料（lit等）は空配列で返る。
    response = client.get("/api/material-catalog")

    entries_by_id = {entry["material_id"]: entry for entry in response.json()["materials"]}

    wind_penalty = entries_by_id["wind_penalty"]
    assert len(wind_penalty["reference_points"]) > 0
    assert wind_penalty["reference_points"][0].keys() == {"label", "value"}
    assert [p.value for p in MATERIAL_CATALOG["wind_penalty"].reference_points] == [
        entry["value"] for entry in wind_penalty["reference_points"]
    ]
