"""CI専用: DB（DATABASE_URL）を一旦まっさらにした上で基本スキーマ作成＋全migration適用を
行う（改善計画T350）。
...
ブートストラップテストが検証する経路と同一）を先に通す必要がある。CIの`backend`
...
適用済みと記録しているが実データ（axis_definitionsの14行）は別テストのTRUNCATEで
